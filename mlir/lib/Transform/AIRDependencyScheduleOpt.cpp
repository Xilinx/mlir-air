...
public:
  ...
  air::LaunchOp cloneAndSpecializeLaunch(air::LaunchOp origLaunch,
    ...
    for (unsigned i = 0; i < numDims; i++) {
      if (i == 0)
        ...
        newSizes[i] = origSzOpt ? *origSzOpt : 1;
      }
    }
  ...
  void runOnOperation() override {
    ...
    for (auto launchOp : launchOps) {
      ...
      // Fully unroll the launch: create one 1×1×1 launch per block.
      // Multi-iteration launches within multi-launch ELF mode don't work,
      // so we need each block to be its own launch.
  ...
  // L1. Does NOT modify L3-level ops — the host still provides zero-padded
  // buffers. The memtile DMA reads actual data from L2 and pads zeros.
  void addMemtilePaddingToLaunch(air::LaunchOp launch, bool padM, bool padN,
    ...
    launch.walk([&](air::ChannelPutOp putOp) {
      ...
      if (padBlocks <= 0)
        return;
      // Hardware limit: memtile DMA padding per dimension is max 31.
      if (padBlocks > 31)
        return;
