...
struct MemcpyBundleAsFlow {
  ...
  int flow_op_group = -1; // Scheduling group index; (in scheduling strategy 2, flows of the same index can share DMA channels)
  ...
  for (auto &f : memcpy_flows) {
    if (f.MM2S_memspace_as_int == (int)air::MemorySpace::L1) {
      for (auto o : f.MM2S) {
        ...
        f.MM2S_alloc =
            tile_dma_alloc.simpleDmaChannelAlloc(memcpyOpIf, x, y, f.MM2S_alloc.dma_channel.second);
    ...
    if (f.S2MM_memspace_as_int == (int)air::MemorySpace::L1) {
      for (int i = 0; i < f.S2MM.size(); i++){
        for (auto o : f.S2MM[i]) {
          ...
          f.S2MM_alloc[i] =
              tile_dma_alloc.simpleDmaChannelAlloc(memcpyOpIf, x, y, f.S2MM_alloc[i].dma_channel.second);
...
// Only those air channel puts and gets which share the same for loop region can share the same AIE DMA channel
bool groupingMemcpysByLoop(std::vector<MemcpyBundleAsFlow> &memcpy_flows){
  ...
  std::map<AIE::CoreOp, std::vector<scf::ForOp>> for_loops_log_mm2s, for_loops_log_s2mm;
  for (auto &f : memcpy_flows) {
    if (f.MM2S_memspace_as_int == (int)air::MemorySpace::L1) {
      auto core = f.MM2S[0]->getParentOfType<AIE::CoreOp>();
      for (auto o : f.MM2S) {
        f.flow_op_group = foundInVector<scf::ForOp>(o->getParentOfType<scf::ForOp>(), for_loops_log_mm2s[core]);
    ...
    if (f.S2MM_memspace_as_int == (int)air::MemorySpace::L1) {
      ...
      for (int i = 0; i < f.S2MM.size(); i++){
        for (auto o : f.S2MM[i]) {
          f.flow_op_group = foundInVector<scf::ForOp>(o->getParentOfType<scf::ForOp>(), for_loops_log_s2mm[core]);
          if (f.flow_op_group == for_loops_log_s2mm[core].size()){
            for_loops_log_s2mm[core].push_back(o->getParentOfType<scf::ForOp>());
          }
        }
      }
    }
...
void groupedByLoopDMAChannelAllocation(std::vector<MemcpyBundleAsFlow> &memcpy_flows,
                                    ShimDMAAllocator &shim_dma_alloc,
                                    MemTileDMAAllocator &memtile_dma_alloc,
                                    TileDMAAllocator &tile_dma_alloc){
  ...
  for (auto &f : memcpy_flows) {
    if (f.MM2S_memspace_as_int == (int)air::MemorySpace::L1) {
      for (auto o : f.MM2S) {
        ...
        f.MM2S_alloc =
            tile_dma_alloc.simpleDmaChannelAlloc(memcpyOpIf, x, y, f.flow_op_group);
        assert(f.MM2S_alloc.dma_tile);
      }
    } 
    if (f.S2MM_memspace_as_int == (int)air::MemorySpace::L1) {
