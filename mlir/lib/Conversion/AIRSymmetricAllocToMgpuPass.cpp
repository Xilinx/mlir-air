...
//===-----------------------------------------------------------------------===//
//
// Lower `memref.alloc` whose result memref carries the `#air.symmetric_heap`
// memory_space to a call to the runtime function `mgpuSymmetricAlloc`. The
// returned `!llvm.ptr` is wrapped in an LLVM memref descriptor (struct) and
// projected back to the original memref type via
// `builtin.unrealized_conversion_cast` so that downstream uses keep working.
//
...
struct AIRSymmetricAllocToMgpuPass
  ...
  void runOnOperation() override {
    ...
    // Collect symmetric allocs (memref result type has #air.symmetric_heap
    // as memory_space).
    ...
    for (memref::AllocOp alloc : symAllocs) {
      ...
      if (!sizeBytes) {
        alloc.emitOpError(
            "memref.alloc with #air.symmetric_heap memory_space requires a "
            "static-shape memref with byte-aligned element type");
        signalPassFailure();
        return;
      }
