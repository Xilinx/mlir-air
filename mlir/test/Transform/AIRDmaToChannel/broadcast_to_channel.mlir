...
// CHECK: %[[EVENT4:.*]] = air.channel.put async{{.*}}@channel_0[]
// CHECK: %[[EVENT5:.*]] = air.channel.put async{{.*}}@channel_1[]
// CHECK: %[[EVENT6:.*]] = air.herd @herd_0 async{{.*}}tile (%[[VALUE2:.*]], %[[VALUE3:.*]]) in
// CHECK: %[[EVENT7:.*]] = air.channel.get async{{.*}}@channel_0[%[[VALUE2]], %[[VALUE3]]]
// CHECK: %[[EVENT8:.*]] = air.channel.get async{{.*}}@channel_1[%[[VALUE2]], %[[VALUE3]]]
          %4 = air.herd @herd_0 async  tile (%arg13, %arg14) in (%arg15=%c2, %arg16=%c2) args(%arg17=%results_1) : memref<64x64xbf16, 1> attributes {id = 1 : i32} {
            ...
            %7 = scf.for %arg18 = %c0_3 to %c64_4 step %c32 iter_args(%arg19 = %6) -> (!air.async.token) {
              ...
              %8 = affine.if #set()[%arg13, %arg14] -> !air.async.token {
                ...
                %10 = air.dma_memcpy_nd async [%async_token_7, %arg19] (%results_8[] [] [], %arg17[%c0_10, %arg18] [%c32, %c32] [%c64_4, %c1]) {broadcast_set = #set, id = 1 : i32} : (memref<32x32xbf16, 2>, memref<64x64xbf16, 1>)
                affine.yield %10 : !air.async.token
              } else {
                %c32_10 = arith.constant 32 : index
                %10 = air.dma_memcpy_nd async [%async_token_7, %arg19] (%results_8[] [] [], %arg17[%c32_10, %arg18] [%c32, %c32] [%c64_4, %c1]) {broadcast_set = #set1, id = 2 : i32} : (memref<32x32xbf16, 2>, memref<64x64xbf16, 1>)
                affine.yield %10 : !air.async.token
              }
