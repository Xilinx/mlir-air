...
// =============================================================================
// Regression: scf.for whose channel op has maxNumDims=4 active wrap dims AND
// the new outer dim WOULD collapse adjacent dim into a value that exceeds the
// downstream airrt-to-npu outer-wrap limit (AIE2_WRAP_UPPER_BOUNDS[0]=64).
// Without the post-fold outer-dim cap, the fold would emit a single channel
// op with outer wrap = 128, which airrt-to-npu then tiles into 4 sibling
// shim BDs — but the matching npu.dma_wait is not replicated, so only 1 of
// 4 BDs gets freed and the per-channel BD pool (~16) leaks.
//
// Concretely: existing wrap <32, 4096> · <8, 64> · <8, 512> · <16, 1>
// (12544 B at dim 0), loop adds outer with stride 131072. Canonicalize
// collapses <4, 131072> · <32, 4096> -> <128, 4096> (4D). The cap kicks
// in (128 >= 64) and rejects the fold, leaving the loop to be unrolled
// into 4 sibling channel ops, each with its own wait_all.
// =============================================================================

// CHECK-LABEL: @postfold_outer_dim_exceeds_shim_limit
// CHECK-NOT:   scf.for
// CHECK:       air.channel.put async {{.*}} @channel_cap[] (%{{.*}}[%c0{{.*}}, %c0{{.*}}, %c0{{.*}}, %c0{{.*}}] [%c32{{.*}}, %c8{{.*}}, %c8{{.*}}, %c16{{.*}}] [%c4096{{.*}}, %c64{{.*}}, %c512{{.*}}, %c1{{.*}}])
// CHECK:       air.channel.put async {{.*}} @channel_cap[] (%{{.*}}[%c32{{.*}}, %c0{{.*}}, %c0{{.*}}, %c0{{.*}}] [%c32{{.*}}, %c8{{.*}}, %c8{{.*}}, %c16{{.*}}] [%c4096{{.*}}, %c64{{.*}}, %c512{{.*}}, %c1{{.*}}])
// CHECK:       air.channel.put async {{.*}} @channel_cap[] (%{{.*}}[%c64{{.*}}, %c0{{.*}}, %c0{{.*}}, %c0{{.*}}] [%c32{{.*}}, %c8{{.*}}, %c8{{.*}}, %c16{{.*}}] [%c4096{{.*}}, %c64{{.*}}, %c512{{.*}}, %c1{{.*}}])
// CHECK:       air.channel.put async {{.*}} @channel_cap[] (%{{.*}}[%c96{{.*}}, %c0{{.*}}, %c0{{.*}}, %c0{{.*}}] [%c32{{.*}}, %c8{{.*}}, %c8{{.*}}, %c16{{.*}}] [%c4096{{.*}}, %c64{{.*}}, %c512{{.*}}, %c1{{.*}}])
// CHECK-NOT:   %c128{{.*}}, %c{{.*}}, %c{{.*}}, %c{{.*}}, %c{{.*}}
// CHECK:       {air.segment_end}

module {
  air.channel @channel_cap [1]
  func.func @postfold_outer_dim_exceeds_shim_limit() {
    %0 = air.launch async () in () {
      %1 = air.segment @seg_cap async {
        %c0 = arith.constant 0 : index
        %c1 = arith.constant 1 : index
        %c8 = arith.constant 8 : index
        %c16 = arith.constant 16 : index
        ...
        %async_token, %results = air.execute -> (memref<524288xbf16, 1 : i32>) {
          %alloc = memref.alloc() : memref<524288xbf16, 1 : i32>
          air.execute_terminator %alloc : memref<524288xbf16, 1 : i32>
        }
        %t = scf.for %iv = %c0 to %c128 step %c32 iter_args(%arg = %async_token) -> (!air.async.token) {
          %p = air.channel.put async [%arg] @channel_cap[] (%results[%iv, %c0, %c0, %c0] [%c32, %c8, %c8, %c16] [%c4096, %c64, %c512, %c1]) {id = 3 : i32} : (memref<524288xbf16, 1 : i32>)
          scf.yield %p : !air.async.token
        }
        %async_token_0 = air.execute {
          memref.dealloc %results : memref<524288xbf16, 1 : i32>
        }
      }
    }
    return
