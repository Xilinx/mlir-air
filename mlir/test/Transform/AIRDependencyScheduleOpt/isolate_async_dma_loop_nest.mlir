...
// Hoisting air.herd out of scf.for, with for-loop induction variables used in herd body.

// CHECK-LABEL: func.func @func10
// CHECK: air.herd @herd_0 async  tile (%{{.*}}, %{{.*}}) in (%{{.*}}=%c4{{.*}}, %{{.*}}=%c4{{.*}}) args(%{{.*}}=%{{.*}}, %{{.*}}=%{{.*}}, %{{.*}}=%{{.*}})
// CHECK: scf.for %[[ARG0:.*]] = %c0{{.*}} to %c4096{{.*}} step %c128{{.*}} {
// CHECK-NEXT: scf.for %[[ARG1:.*]] = %c0{{.*}} to %c1024{{.*}} step %c128{{.*}} {
// CHECK-NEXT: scf.for %[[ARG2:.*]] = %c0{{.*}} to %c512{{.*}} step %c128{{.*}} {
// CHECK: arith.addi %[[ARG0]], %{{.*}}

#map0 = affine_map<()[s0, s1] -> (s0 * 64 + s1 * 128)>
#map1 = affine_map<(d0)[s0] -> (d0 + s0 * 16)>
module  {
  func.func @func10(%arg0: memref<4096x1024x512xi32>, %arg1: memref<4096x1024x512xi32>, %arg2: memref<4096x1024x512xi32>) {
    %c4 = arith.constant 4 : index
...
// Split scf.for loop body based on async dependency graph partitioning.

// CHECK: scf.for %{{.*}} = %c0{{.*}} to %c512{{.*}} step %c256{{.*}}
// CHECK: scf.for %{{.*}} = %c0{{.*}} to %c512{{.*}} step %c256{{.*}} iter_args(%[[VALUE0:.*]] = %{{.*}})
// CHECK: %[[GET0:.*]] = air.channel.get async [%[[VALUE0]]]  @channel_10[%c0
// CHECK: %[[GET1:.*]] = air.channel.get async [%[[VALUE0]]]  @channel_10[%c0
// CHECK: %[[GET2:.*]] = air.channel.get async [%[[VALUE0]]]  @channel_10[%c0
// CHECK: %[[GET3:.*]] = air.channel.get async [%[[VALUE0]]]  @channel_10[%c0
// CHECK: %[[PUT0:.*]] = air.channel.put async [%[[GET0]], %[[GET1]], %[[GET2]], %[[GET3]]]  @channel_12
// CHECK: scf.yield %[[PUT0]] : !air.async.token

// CHECK: scf.for %{{.*}} = %c0{{.*}} to %c512{{.*}} step %c256{{.*}}
// CHECK: scf.for %{{.*}} = %c0{{.*}} to %c512{{.*}} step %c256{{.*}} iter_args(%[[VALUE0:.*]] = %{{.*}})
// CHECK: %[[GET0:.*]] = air.channel.get async [%[[VALUE0]]]  @channel_10[%c1
// CHECK: %[[GET1:.*]] = air.channel.get async [%[[VALUE0]]]  @channel_10[%c1
// CHECK: %[[GET2:.*]] = air.channel.get async [%[[VALUE0]]]  @channel_10[%c1
// CHECK: %[[GET3:.*]] = air.channel.get async [%[[VALUE0]]]  @channel_10[%c1
// CHECK: %[[PUT0:.*]] = air.channel.put async [%[[GET0]], %[[GET1]], %[[GET2]], %[[GET3]]]  @channel_12
// CHECK: scf.yield %[[PUT0]] : !air.async.token
...
module {
  ...
  func.func @func11() {
    %c1 = arith.constant 1 : index
    %0 = air.launch async (%arg0) in (%arg1=%c1) attributes {id = 1 : i32} {
      %1 = air.segment @segment_0 async  attributes {id = 2 : i32} {
        %c3 = arith.constant 3 : index
        %c2 = arith.constant 2 : index
        %c4096 = arith.constant 4096 : index
        ...
        %c512 = arith.constant 512 : index
        %c256 = arith.constant 256 : index
        %async_token, %results = air.execute -> (memref<1x4x64x64xbf16, 1>) {
          %alloc = memref.alloc() : memref<1x4x64x64xbf16, 1>
          air.execute_terminator %alloc : memref<1x4x64x64xbf16, 1>
        }
        %async_token_1, %results_2 = air.execute -> (memref<1x4x64x64xbf16, 1>) {
          %alloc = memref.alloc() : memref<1x4x64x64xbf16, 1>
          air.execute_terminator %alloc : memref<1x4x64x64xbf16, 1>
        }
        %2 = air.wait_all async [%async_token, %async_token_1] 
        %3 = scf.for %arg2 = %c0 to %c512 step %c256 iter_args(%arg3 = %2) -> (!air.async.token) {
          %4 = scf.for %arg4 = %c0 to %c512 step %c256 iter_args(%arg5 = %arg3) -> (!air.async.token) {
            %5 = air.channel.get async [%arg5]  @channel_10[%c0, %c0] (%results[%c0, %c0, %c0, %c0] [%c1_0, %c1_0, %c64, %c64] [%c16384, %c4096, %c64, %c1_0]) {id = 38 : i32} : (memref<1x4x64x64xbf16, 1>)
            %6 = air.channel.get async [%arg5]  @channel_10[%c1_0, %c0] (%results_2[%c0, %c0, %c0, %c0] [%c1_0, %c1_0, %c64, %c64] [%c16384, %c4096, %c64, %c1_0]) {id = 39 : i32} : (memref<1x4x64x64xbf16, 1>)
            %7 = air.channel.get async [%arg5]  @channel_10[%c0, %c1_0] (%results[%c0, %c1_0, %c0, %c0] [%c1_0, %c1_0, %c64, %c64] [%c16384, %c4096, %c64, %c1_0]) {id = 42 : i32} : (memref<1x4x64x64xbf16, 1>)
            %8 = air.channel.get async [%arg5]  @channel_10[%c1_0, %c1_0] (%results_2[%c0, %c1_0, %c0, %c0] [%c1_0, %c1_0, %c64, %c64] [%c16384, %c4096, %c64, %c1_0]) {id = 43 : i32} : (memref<1x4x64x64xbf16, 1>)
            %9 = air.channel.get async [%arg5]  @channel_10[%c0, %c2] (%results[%c0, %c2, %c0, %c0] [%c1_0, %c1_0, %c64, %c64] [%c16384, %c4096, %c64, %c1_0]) {id = 46 : i32} : (memref<1x4x64x64xbf16, 1>)
            %10 = air.channel.get async [%arg5]  @channel_10[%c1_0, %c2] (%results_2[%c0, %c2, %c0, %c0] [%c1_0, %c1_0, %c64, %c64] [%c16384, %c4096, %c64, %c1_0]) {id = 47 : i32} : (memref<1x4x64x64xbf16, 1>)
            %11 = air.channel.get async [%arg5]  @channel_10[%c0, %c3] (%results[%c0, %c3, %c0, %c0] [%c1_0, %c1_0, %c64, %c64] [%c16384, %c4096, %c64, %c1_0]) {id = 50 : i32} : (memref<1x4x64x64xbf16, 1>)
            %12 = air.channel.get async [%arg5]  @channel_10[%c1_0, %c3] (%results_2[%c0, %c3, %c0, %c0] [%c1_0, %c1_0, %c64, %c64] [%c16384, %c4096, %c64, %c1_0]) {id = 51 : i32} : (memref<1x4x64x64xbf16, 1>)
            %13 = air.channel.put async [%5, %7, %9, %11]  @channel_12[%c0, %c0] (%results[%c0, %c0, %c0, %c0] [%c1_0, %c64, %c4, %c64] [%c16384, %c64, %c4096, %c1_0]) {id = 54 : i32} : (memref<1x4x64x64xbf16, 1>)
            %14 = air.channel.put async [%6, %8, %10, %12]  @channel_12[%c1_0, %c0] (%results_2[%c0, %c0, %c0, %c0] [%c1_0, %c64, %c4, %c64] [%c16384, %c64, %c4096, %c1_0]) {id = 55 : i32} : (memref<1x4x64x64xbf16, 1>)
        ...
        %async_token_3 = air.execute {
          memref.dealloc %results_2 : memref<1x4x64x64xbf16, 1>
        }
        %async_token_4 = air.execute {
          memref.dealloc %results : memref<1x4x64x64xbf16, 1>
        }
      }
    }
    return
...
// CHECK-NEXT: }
// CHECK: scf.for
// CHECK-NEXT: air.channel.put{{.*}}@channel_12
// CHECK: scf.for
// CHECK-NEXT: air.channel.put{{.*}}@channel_13
// CHECK: scf.for
// CHECK-NEXT: air.channel.put{{.*}}@channel_14
// CHECK: scf.for
// CHECK-NEXT: air.channel.put{{.*}}@channel_15
// CHECK: scf.for
// CHECK-NEXT: air.channel.put{{.*}}@channel_16
// CHECK: scf.for
// CHECK-NEXT: air.channel.put{{.*}}@channel_17
// CHECK: scf.for
// CHECK-NEXT: air.channel.put{{.*}}@channel_18
// CHECK: scf.for
// CHECK-NEXT: air.channel.put{{.*}}@channel_19
// CHECK: air.herd @herd_0
// CHECK: scf.for
// CHECK: affine.if
...
module {
  ...
  air.channel @channel_16 [1, 1] {broadcast_shape = [4, 1]}
  air.channel @channel_17 [1, 1] {broadcast_shape = [4, 1]}
  air.channel @channel_18 [1, 1] {broadcast_shape = [4, 1]}
  air.channel @channel_19 [1, 1] {broadcast_shape = [4, 1]}
  func.func @func15(%arg0: memref<512x512xi32>, %arg1: memref<512x512xi32>, %arg2: memref<512x512xi32>) {
    %c4 = arith.constant 4 : index
    %0 = air.launch async (%arg3, %arg4) in (%arg5=%c4, %arg6=%c4) {
      %1 = air.segment @seg async  attributes {id = 2 : i32} {
        ...
        %async_token_1, %results_2 = air.execute -> (memref<1x4x32x32xi32, 1 : i32>) {
          %alloc = memref.alloc() : memref<1x4x32x32xi32, 1 : i32>
          air.execute_terminator %alloc : memref<1x4x32x32xi32, 1 : i32>
        }
        %async_token_3, %results_4 = air.execute -> (memref<4x1x32x32xi32, 1 : i32>) {
          %alloc = memref.alloc() : memref<4x1x32x32xi32, 1 : i32>
          air.execute_terminator %alloc : memref<4x1x32x32xi32, 1 : i32>
        }
        %2 = air.wait_all async [%async_token_1, %async_token_3] 
        %3 = scf.for %arg7 = %c1 to %c15 step %c1 iter_args(%arg8 = %2) -> (!air.async.token) {
          %4 = air.channel.put async  @channel_12[] (%results_4[%c0, %c0, %c0, %c0, %c0, %c0] [%c1, %c1, %c4_0, %c8, %c4_0, %c8] [%c1024, %c1024, %c8, %c128, %c32, %c1]) {broadcast_set = #set, id = 28 : i32} : (memref<4x1x32x32xi32, 1 : i32>)
          %5 = air.channel.put async  @channel_13[] (%results_4[%c1, %c0, %c0, %c0, %c0, %c0] [%c1, %c1, %c4_0, %c8, %c4_0, %c8] [%c1024, %c1024, %c8, %c128, %c32, %c1]) {broadcast_set = #set1, id = 29 : i32} : (memref<4x1x32x32xi32, 1 : i32>)
          %6 = air.channel.put async  @channel_14[] (%results_4[%c2, %c0, %c0, %c0, %c0, %c0] [%c1, %c1, %c4_0, %c8, %c4_0, %c8] [%c1024, %c1024, %c8, %c128, %c32, %c1]) {broadcast_set = #set2, id = 30 : i32} : (memref<4x1x32x32xi32, 1 : i32>)
          %7 = air.channel.put async  @channel_15[] (%results_4[%c3, %c0, %c0, %c0, %c0, %c0] [%c1, %c1, %c4_0, %c8, %c4_0, %c8] [%c1024, %c1024, %c8, %c128, %c32, %c1]) {broadcast_set = #set3, id = 31 : i32} : (memref<4x1x32x32xi32, 1 : i32>)
          %8 = air.channel.put async  @channel_16[] (%results_2[%c0, %c0, %c0, %c0, %c0, %c0] [%c1, %c1, %c8, %c4_0, %c8, %c4_0] [%c4096, %c1024, %c4_0, %c256, %c32, %c1]) {broadcast_set = #set4, id = 32 : i32} : (memref<1x4x32x32xi32, 1 : i32>)
          %9 = air.channel.put async  @channel_17[] (%results_2[%c0, %c1, %c0, %c0, %c0, %c0] [%c1, %c1, %c8, %c4_0, %c8, %c4_0] [%c4096, %c1024, %c4_0, %c256, %c32, %c1]) {broadcast_set = #set5, id = 33 : i32} : (memref<1x4x32x32xi32, 1 : i32>)
          %10 = air.channel.put async  @channel_18[] (%results_2[%c0, %c2, %c0, %c0, %c0, %c0] [%c1, %c1, %c8, %c4_0, %c8, %c4_0] [%c4096, %c1024, %c4_0, %c256, %c32, %c1]) {broadcast_set = #set6, id = 34 : i32} : (memref<1x4x32x32xi32, 1 : i32>)
          %11 = air.channel.put async  @channel_19[] (%results_2[%c0, %c3, %c0, %c0, %c0, %c0] [%c1, %c1, %c8, %c4_0, %c8, %c4_0] [%c4096, %c1024, %c4_0, %c256, %c32, %c1]) {broadcast_set = #set7, id = 35 : i32} : (memref<1x4x32x32xi32, 1 : i32>)
          %12 = air.herd @herd_0 async [%arg8]  tile (%arg9, %arg10) in (%arg11=%c4_0, %arg12=%c4_0) args(%arg13=%results) : memref<1x1x4x8x4x8xi32, 2 : i32> attributes {id = 4 : i32, link_with = "mm.o"} {
            %13 = affine.if #set()[%arg9, %arg10] -> !air.async.token {
              %14 = air.channel.get async  @channel_12[%arg9, %arg10] (%arg13[] [] []) {id = 36 : i32} : (memref<1x1x4x8x4x8xi32, 2 : i32>)
              affine.yield %14 : !air.async.token
            } else {
              %14 = affine.if #set1()[%arg9, %arg10] -> !air.async.token {
                %15 = air.channel.get async  @channel_13[%arg9, %arg10] (%arg13[] [] []) {id = 37 : i32} : (memref<1x1x4x8x4x8xi32, 2 : i32>)
                affine.yield %15 : !air.async.token
              } else {
                %15 = affine.if #set2()[%arg9, %arg10] -> !air.async.token {
                  %16 = air.channel.get async  @channel_14[%arg9, %arg10] (%arg13[] [] []) {id = 38 : i32} : (memref<1x1x4x8x4x8xi32, 2 : i32>)
                  affine.yield %16 : !air.async.token
                } else {
                  %16 = air.channel.get async  @channel_15[%arg9, %arg10] (%arg13[] [] []) {id = 39 : i32} : (memref<1x1x4x8x4x8xi32, 2 : i32>)
                  affine.yield %16 : !air.async.token
                }
                affine.yield %15 : !air.async.token
              }
