...
// CHECK:  %[[VAL_13:.*]] = memref.subview %{{.*}}[0, %{{.*}}, %{{.*}}, %{{.*}} [1, 16, 18, 18] [1, 1, 1, 1] : memref<1x64x66x66xf32> to memref<1x16x18x18xf32, #{{.*}}>
// CHECK:  %[[VAL_14:.*]] = memref.subview %{{.*}}{{\[}}%{{.*}}, %{{.*}}, 0, 0] [32, 16, 3, 3] [1, 1, 1, 1] : memref<128x64x3x3xf32> to memref<32x16x3x3xf32, #{{.*}}>
// CHECK:  %[[VAL_15:.*]] = memref.subview %{{.*}}[0, %{{.*}}, %{{.*}}, %{{.*}} [1, 32, 16, 16] [1, 1, 1, 1] : memref<1x128x64x64xf32> to memref<1x32x16x16xf32, #{{.*}}>
// CHECK:  %[[VAL_16:.*]] = memref.alloc() : memref<1x16x18x18xf32, 1 : i32>
// CHECK:  %[[VAL_17:.*]] = memref.alloc() : memref<32x16x3x3xf32, 1 : i32>
// CHECK:  %[[VAL_18:.*]] = memref.alloc() : memref<1x32x16x16xf32, 1 : i32>
// CHECK:  linalg.copy ins(%[[VAL_13]] : memref<1x16x18x18xf32, #{{.*}}>) outs(%[[VAL_16]] : memref<1x16x18x18xf32, 1 : i32>)
// CHECK:  linalg.copy ins(%[[VAL_14]] : memref<32x16x3x3xf32, #{{.*}}>) outs(%[[VAL_17]] : memref<32x16x3x3xf32, 1 : i32>)
// CHECK:  linalg.copy ins(%[[VAL_15]] : memref<1x32x16x16xf32, #{{.*}}>) outs(%[[VAL_18]] : memref<1x32x16x16xf32, 1 : i32>)
// CHECK:  linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<1> : vector<2xi64>} ins(%[[VAL_16]], %[[VAL_17]] : memref<1x16x18x18xf32, 1 : i32>, memref<32x16x3x3xf32, 1 : i32>) outs(%[[VAL_18]] : memref<1x32x16x16xf32, 1 : i32>)
// CHECK:  linalg.copy ins(%[[VAL_18]] : memref<1x32x16x16xf32, 1 : i32>) outs(%[[VAL_15]] : memref<1x32x16x16xf32, #{{.*}}>)
// CHECK:  memref.dealloc %[[VAL_16]] : memref<1x16x18x18xf32, 1 : i32>
// CHECK:  memref.dealloc %[[VAL_17]] : memref<32x16x3x3xf32, 1 : i32>
// CHECK:  memref.dealloc %[[VAL_18]] : memref<1x32x16x16xf32, 1 : i32>
