...
// CHECK: memref.copy {{.*}} : memref<{{.*}}> to memref<64x64xi32, 1 : i32>
// CHECK: memref.copy {{.*}} : memref<{{.*}}> to memref<64x64xi32, 1 : i32>
// CHECK: memref.copy {{.*}} : memref<{{.*}}> to memref<64x64xi32, 1 : i32>
// CHECK: scf.parallel ({{.*}}) = (%c0, %c0) to (%c64, %c64) step (%c32, %c32) {
// CHECK: scf.for {{.*}} = %c0 to %c64 step %c32 {
// CHECK: memref.copy {{.*}} : memref<{{.*}}, 1 : i32> to memref<{{.*}}, 2 : i32>
// CHECK: memref.copy {{.*}} : memref<{{.*}}, 1 : i32> to memref<{{.*}}, 2 : i32>
// CHECK: memref.copy {{.*}} : memref<{{.*}}, 1 : i32> to memref<{{.*}}, 2 : i32>
// CHECK: memref.copy {{.*}} : memref<{{.*}}, 2 : i32> to memref<{{.*}}, 1 : i32>
// CHECK: scf.reduce
// CHECK: memref.copy {{.*}} : memref<{{.*}}, 1 : i32> to memref<{{.*}}>
