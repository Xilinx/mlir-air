//===- split_launch_for_padding_source.mlir --------------------*- MLIR -*-===//
//
...
// Tests pad-location=source with both DMA and channel ops.
// The deprecated use-dma-memcpy=true flag should produce identical output.

// -----

// Test 1: Source-level padding with DMA ops, M and N both unaligned.
...
// Test 2: Source-level padding with DMA ops, only N unaligned.
// M=512 (tile-aligned with tileM=64, launchM=8), actualM=0 means no M padding
// N=500, N_TILE=32 -> launchN=16, last N-block has 500-15*32=20 cols
// Only N needs padding -> 2 partitions: interior 8x15, n_boundary 8x1

// CHECK-LABEL: func.func @matmul_n_only

// Interior: no padding.
// CHECK: air.segment @matmul_n_only_0_interior
// CHECK: air.dma_memcpy_nd ({{.*}}) {id = 1 : i32}
// CHECK-NOT: pad_after
// CHECK: air.dma_memcpy_nd ({{.*}}) {id = 2 : i32}
// CHECK-NOT: pad_after

// No m_boundary or corner partitions.
// CHECK-NOT: _m_boundary
// CHECK-NOT: _corner

// N-boundary: A DMA unchanged, B DMA has reduced src_sizes and pad_after.
// CHECK: air.segment @matmul_n_only_0_n_boundary
// CHECK: air.dma_memcpy_nd ({{.*}}) {id = 1 : i32}
// CHECK-NOT: pad_after
// CHECK: air.dma_memcpy_nd ({{.*}}) {id = 2 : i32, pad_after = array<i32: 0, 12>, pad_before = array<i32: 0, 0>}
...
// Test 3: Source-level padding with channel ops.
// M=500, M_TILE=64 -> launchM=8, last M-block has 500-7*64=52 rows
// N=500, N_TILE=32 -> launchN=16, last N-block has 500-15*32=20 cols
// Expected: 4 partitions (interior 7x15, m_boundary 1x15, n_boundary 7x1, corner 1x1)
...
module {
  ...
  func.func @matmul_n_only(%arg0: memref<*xbf16>, %arg1: memref<*xbf16>, %arg2: memref<*xbf16>) {
    ...
    %c8 = arith.constant 8 : index
    %c16 = arith.constant 16 : index
    air.launch (%arg9, %arg10, %arg11) in (%arg12=%c8, %arg13=%c16, %arg14=%c1) args(%arg15=%arg0, %arg16=%arg1, %arg17=%arg2) : memref<*xbf16>, memref<*xbf16>, memref<*xbf16> attributes {air.actual_sizes = array<i64: 0, 500, 1>} {
      air.segment @matmul_n_only_0  args(%arg18=%arg9, %arg19=%arg10, %arg20=%arg15, %arg21=%arg16, %arg22=%arg17) : index, index, memref<*xbf16>, memref<*xbf16>, memref<*xbf16> {
        %c0 = arith.constant 0 : index
        %c1_0 = arith.constant 1 : index
        %c16_1 = arith.constant 16 : index
        %c32000 = arith.constant 32000 : index
