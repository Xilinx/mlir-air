//===- bad_partition_per_core.mlir -----------------------------*- MLIR -*-===//
//
...
// Check for error emission when not enough resources are allocated to run a partition.
// Trace simulation mode set to per-core granularity.

// CHECK: error: 'air.partition' op isn't allocated with enough resources to run
