...
#define M_SIZE 32
  ...
  input_A.alloc = input_A.data = static_cast<uint32_t *>(
      malloc(sizeof(uint32_t) * input_A.shape[0] * input_A.shape[1]));

  input_B.shape[0] = input_B.shape[1] = M_SIZE;
  input_B.alloc = input_B.data = static_cast<uint32_t *>(
      malloc(sizeof(uint32_t) * input_B.shape[0] * input_B.shape[1]));

  input_C.shape[0] = input_C.shape[1] = M_SIZE;
  input_C.alloc = input_C.data = static_cast<uint32_t *>(
      malloc(sizeof(uint32_t) * input_C.shape[0] * input_C.shape[1]));

  output.shape[0] = output.shape[1] = M_SIZE;
  output.alloc = output.data = static_cast<uint32_t *>(
      malloc(sizeof(uint32_t) * output.shape[0] * output.shape[1]));

  output_ref0.shape[0] = output_ref0.shape[1] = M_SIZE;
  output_ref0.alloc = output_ref0.data = static_cast<uint32_t *>(
      malloc(sizeof(uint32_t) * output_ref0.shape[0] * output_ref0.shape[1]));

  output_ref1.shape[0] = output_ref1.shape[1] = M_SIZE;
  output_ref1.alloc = output_ref1.data = static_cast<uint32_t *>(
      malloc(sizeof(uint32_t) * output_ref1.shape[0] * output_ref1.shape[1]));

  for (unsigned i = 0; i < input_A.shape[0] * input_A.shape[1]; i++) {
    input_A.data[i] = (static_cast<uint32_t>(i)) % 3;
    input_B.data[i] = (static_cast<uint32_t>(i) + 1) % 5;
    input_C.data[i] = (static_cast<uint32_t>(i) + 2) % 7;
    output.data[i] = 0;
