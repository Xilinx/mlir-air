...
func.func @test(%arg0 : memref<16x16xf32>, %arg1 : memref<16x16xf32>) -> () {
  ...
  // CHECK: air.launch (%{{.*}}, %{{.*}}) in (%{{.*}}=%c1, %{{.*}}=%c2) attributes {dim = 2 : index, foo = "bar"} {
  air.launch (%tx, %ty) in (%size_x = %c1, %size_y = %c2) attributes {foo = "bar"} {
    air.launch_terminator
  }

  // CHECK: air.launch (%{{.*}}, %{{.*}}, %{{.*}}) in (%{{.*}}=%c2, %{{.*}}=%c3, %{{.*}}=%c4) attributes {dim = 3 : index} {
  air.launch (%tx, %ty, %tz) in (%sx = %c2, %sy = %c3, %sz = %c4) attributes {  } {
    air.launch_terminator
  }

  // CHECK: air.launch async (%{{.*}}) in (%{{.*}}=%c1) attributes {dim = 1 : index}
  %t0 = air.launch async (%tx) in (%size_x = %c1) {
    air.launch_terminator
  }

  // CHECK: %{{.*}} = air.launch async [%{{.*}}] (%{{.*}}) in (%{{.*}}=%c2) attributes {dim = 1 : index}
  %t1 = air.launch async [%t0] (%tx) in (%size_x = %c2) {
    air.launch_terminator
  }

  // CHECK: air.launch [%{{.*}}, %{{.*}}] (%{{.*}}) in (%{{.*}}=%c3) attributes {dim = 1 : index}
