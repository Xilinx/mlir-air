//===- memtile_circuit_two_sources_one_dst.mlir ----------------*- MLIR -*-===//
//
...
// Circuit-flow companion to memtile_packet_two_sources_one_dst.mlir. Guards the
// NO-OVER-COLLAPSE side of the S2MM discrimination in
// MemTileDMAAllocator::simpleDmaChannelAlloc: extending the same-logical-flow
// collapse to circuit flows must NOT merge distinct sources. Each distinct
// (channel symbol, indices) endpoint is a different logical flow and must get
// its own physical S2MM channel; only multiple invocations of the SAME flow may
// share one (see memtile_circuit_same_flow_collapse.mlir).
//
// Here two distinct circuit channels (@w0, @w1) each write a disjoint sub-region
// of one L2 buffer at the receiver memtile. isSameLogicalFlowEndpoint requires
// matching channel decls, so the two land on TWO separate S2MM channels.
...
air.channel @w1 [1, 1]
air.channel @r0 [1, 1]
func.func @memtile_circ_two_src() {
  ...
  air.launch (%a, %b) in (%c=%c1, %d=%c1) {
    air.segment @seg {
      ...
      // Two distinct circuit channels write disjoint rows of the same L2 buffer.
