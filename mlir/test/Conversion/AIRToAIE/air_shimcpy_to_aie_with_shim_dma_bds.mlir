...
// CHECK: AIE.device
// CHECK:         %[[VAL_6:.*]] = AIE.external_buffer {sym_name = {{.*}}} : memref<512xi32>
// CHECK:         %[[VAL_7:.*]] = AIE.external_buffer {sym_name = {{.*}}} : memref<1024xi32>
// CHECK:         %[[VAL_8:.*]] = AIE.tile(2, 2)
// CHECK:         %[[VAL_9:.*]] = AIE.tile(2, 0)
// CHECK:         %[[VAL_10:.*]] = AIE.lock(%3, 1) {init = 0 : i32}
// CHECK:         %[[VAL_11:.*]] = AIE.lock(%3, 0) {init = 0 : i32}
// CHECK:         %[[VAL_12:.*]] = AIE.lock(%2, 1) {init = 0 : i32}
// CHECK:         %[[VAL_13:.*]] = AIE.lock(%2, 0) {init = 0 : i32}
// CHECK:         %[[VAL_14:.*]] = AIE.buffer(%2) {sym_name = {{.*}}} : memref<1024xi32, 2>
// CHECK:         %[[VAL_15:.*]] = AIE.buffer(%2) {sym_name = {{.*}}} : memref<512xi32, 2>

// CHECK:    AIE.mem(%[[VAL_8]])  {
// CHECK:           AIE.dmaStart(S2MM, 0, ^bb1, ^bb3)
// CHECK:         ^bb1:
// CHECK:           AIE.useLock(%[[VAL_13]], Acquire, 0)
// CHECK:           AIE.dmaBd(<%[[VAL_14]] : memref<1024xi32, 2>, 0, 1024>, 0)
...
// CHECK:           AIE.dmaBd(<%[[VAL_15]] : memref<512xi32, 2>, 0, 512>, 0)
// CHECK:           AIE.useLock(%[[VAL_12]], Release, 1)
// CHECK:           AIE.nextBd ^bb4
// CHECK:         }

// CHECK:    AIE.core(%[[VAL_8]])  {
...
// CHECK:         AIE.flow(%[[VAL_9]], DMA : 0, %[[VAL_8]], DMA : 0)
// CHECK:         AIE.flow(%[[VAL_9]], DMA : 1, %[[VAL_8]], DMA : 1)

// CHECK:    AIE.shimDMA(%[[VAL_9]])  {
// CHECK:           AIE.dmaStart(MM2S, 0, ^bb1, ^bb3)
// CHECK:         ^bb1:
// CHECK:           AIE.useLock(%[[VAL_11]], Acquire, 1)
// CHECK:           AIE.dmaBd(<%[[VAL_7]] : memref<1024xi32>, 0, 1024>, 0)
...
// CHECK:           AIE.dmaBd(<%[[VAL_6]] : memref<512xi32>, 0, 512>, 0)
...
// CHECK:         %[[VAL_5:.*]] = AIE.lock(%[[VAL_3]], 0)
// CHECK:         %[[VAL_6:.*]] = AIE.lock(%[[VAL_2]], 1)
// CHECK:         %[[VAL_7:.*]] = AIE.lock(%[[VAL_2]], 0)
