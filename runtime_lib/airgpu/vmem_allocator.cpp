...
size_t VMemAllocator::align_up(size_t value, size_t alignment) {
...
VMemAllocator::VMemAllocator(size_t heap_size) {
    ...
    heap_size_ = align_up(heap_size, granularity_);
...
VMemAllocator::~VMemAllocator() {
    ...
    // Free reserved VA
    if (va_base_) {
        hipMemAddressFree(reinterpret_cast<hipDeviceptr_t>(va_base_),
                          heap_size_);
        va_base_ = nullptr;
    }
}
...
void *VMemAllocator::allocate(size_t size_bytes) {
    ...
    size_t aligned_size = align_up(size_bytes, granularity_);
    size_t aligned_offset = align_up(current_offset_, granularity_);
