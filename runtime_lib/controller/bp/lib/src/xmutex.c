...
int XMutex_CfgInitialize(XMutex *InstancePtr, XMutex_Config *ConfigPtr,
			 UINTPTR EffectiveAddress, u32 CpuId)
{
	u32 i;
	...
	for(i=0; i < ConfigPtr->NumMutex; i++){
		XMutex_Unlock(InstancePtr, i, CpuId);
	}
...
*
******************************************************************************/
void XMutex_Lock(XMutex *InstancePtr, u8 MutexNumber, u32 CpuId)
{
	u32 LockPattern = ((CpuId << OWNER_SHIFT) | LOCKED_BIT);
	u32 Value;
...
*
******************************************************************************/
int XMutex_Trylock(XMutex *InstancePtr, u8 MutexNumber, u32 CpuId)
{
	u32 LockPattern = ((CpuId << OWNER_SHIFT) | LOCKED_BIT);
	u32 Value;
...
*
******************************************************************************/
int XMutex_Unlock(XMutex *InstancePtr, u8 MutexNumber, u32 CpuId)
{
	u32 UnLockPattern = ((CpuId << OWNER_SHIFT) | 0x0);
	u32 Value;
