...
#if !defined(__AIESIM__) && !defined(__CARDANO_FRONTEND__)
...
/*
    ...
    //cardano::registerXRT(dhdl, top->m_header.uuid);
...
*/
    ...
    for (int i=M; i<M+P; i++)
    {
        ...
        //gmioIn[i]->init(inputArray[i], 256*sizeof(int32));
        for (int j=0; j<256; j++)
            inputArray[i][j] = i+1;
    }
    ...
    for (int i=M; i<M+P; i++)
    {
        ...
        //gmioOut[i]->init(outputArray[i], 256*sizeof(int32));
        for (int j=0; j<256; j++)
