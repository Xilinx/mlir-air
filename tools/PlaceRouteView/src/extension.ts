...
const CSS_COLOR_NAMES = ['purple', 'cyan', 'yellow', 'orange', 'blue', 'pink', 'red',
"Aqua", "BlueViolet","Brown","BurlyWood", "CadetBlue",
"Chartreuse","Chocolate","Coral","CornflowerBlue", "Crimson","DarkBlue",
"DarkCyan","BlanchedAlmond","Aquamarine","DarkGoldenRod","DarkGray","DarkGrey","DarkGreen","DarkKhaki","DarkMagenta",
"DarkOliveGreen","DarkOrange","DarkOrchid","DarkRed","DarkSalmon","DarkSeaGreen",
"DarkSlateBlue","DarkSlateGray","DarkSlateGrey","DarkTurquoise","DarkViolet",
"DeepPink","DeepSkyBlue","DimGray","DimGrey","DodgerBlue","FireBrick","FloralWhite",
"ForestGreen","Fuchsia", "Gold","GoldenRod","Gray","Grey",
"GreenYellow","HoneyDew","HotPink","IndianRed","Indigo", "Khaki",
"Lavender","LavenderBlush","LawnGreen","LightBlue","LightCoral",
"LightCyan", "LightGray","LightGrey","LightGreen","LightPink",
"LightSalmon","LightSeaGreen","LightSkyBlue","LightSlateGray","LightSlateGrey",
"LightSteelBlue","Lime","LimeGreen","Magenta","Maroon",
"MediumAquaMarine","MediumBlue","MediumOrchid","MediumPurple","MediumSeaGreen",
"MediumSlateBlue","MediumSpringGreen","MediumTurquoise","MediumVioletRed","MidnightBlue",
"MintCream","MistyRose","Moccasin","NavajoWhite","Navy","Olive","OliveDrab",
"OrangeRed","Orchid","PaleGoldenRod","PaleGreen","PaleTurquoise",
"PaleVioletRed","PeachPuff","Peru","Plum","PowderBlue",
"RebeccaPurple","Red","RosyBrown","RoyalBlue","SaddleBrown","Salmon",
"SandyBrown","SeaGreen","Sienna","Silver","SkyBlue","SlateBlue","SlateGray",
"SlateGrey","SpringGreen","SteelBlue","Tan","Teal","Thistle","Tomato","Turquoise",
...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		...
		vscode.commands.registerCommand('prviewer.start', (passedInPart: string | undefined) => {
			const editor = vscode.window.activeTextEditor;
			if (editor === undefined) {
				vscode.window.showInformationMessage(`Try running the command again with a .json file \
			...
			if (passedInPart && editor.document.fileName in openFilePartPanel 
				&& passedInPart in openFilePartPanel[currPartFileName]) {
					openFilePartPanel[currPartFileName][passedInPart].reveal(vscode.ViewColumn.Beside);
			}
			...
			if (passedInPart && !(openFilePartPanel.hasOwnProperty(currPartFileName))) {
				const panel = vscode.window.createWebviewPanel(
					'prviewer',
					`${passedInPart} in ` + currPartFileName.substring(currPartFileName.lastIndexOf("/") + 1),
				...
				update_window(document, openFilePartPanel[currPartFileName][passedInPart], false, true, undefined, json_object, passedInPart);
				return;
			}
			...
			if (!passedInPart) {
				...
				if (!drawName) {
					vscode.window.showInformationMessage("Please add a partition to the file before running the Part command.");
					return;
				}

				if (openFilePartPanel.hasOwnProperty(currPartFileName) && 
					openFilePartPanel[currPartFileName].hasOwnProperty(drawName)) {
						update_window(document, openFilePartPanel[currPartFileName][drawName], false, true, undefined, json_object, drawName);
						return;
				}
				const panel = vscode.window.createWebviewPanel(
					'prviewer',
					`${drawName} in ` + currPartFileName.substring(currPartFileName.lastIndexOf("/") + 1),
					vscode.ViewColumn.Beside,
					{
						// Enable scripts in the webview
						enableScripts: true
		...
		new (class implements vscode.HoverProvider {
		  provideHover(
			...
			if (word.includes("route")) {
				// Encode URI is needed because it passes things in a weird format - can't just
				// do drawRoutes('arg');
				const commentCommandUri = vscode.Uri.parse(`command:prviewer.drawRoutes?${encodeURIComponent(JSON.stringify(word))}`);
				const contents = new vscode.MarkdownString(`[Display ${word}](${commentCommandUri})`);
		...
		new (class implements vscode.HoverProvider {
		  provideHover(
			...
			if (word.includes("part")) {
				// Encode URI is needed because it passes things in a weird format - can't just
				// do start('arg');
				const commentCommandUri = vscode.Uri.parse(`command:prviewer.start?${encodeURIComponent(JSON.stringify(word))}`);
				const contents = new vscode.MarkdownString(`[Display ${word}](${commentCommandUri})`);
	...
	context.subscriptions.push(
		// sets up the webview (visual representation of a partition)
		vscode.commands.registerCommand('prviewer.drawRoutes', (passedInRoute: string | undefined) => {
			const editor = vscode.window.activeTextEditor;
			if (editor === undefined) {
				vscode.window.showInformationMessage(`
				Try running the command again with a .json file as the
			...
			if (passedInRoute && editor.document.fileName in openFileRoutePanel 
				&& passedInRoute in openFileRoutePanel[currRouteFileName]) {
					openFileRoutePanel[currRouteFileName][passedInRoute].reveal(vscode.ViewColumn.Beside);
			}
				//get word within selection
			const json_file = document.getText();
			var json_object: object = {};
			...
			// if we're entering from a new hover event
			if (passedInRoute && !(openFileRoutePanel.hasOwnProperty(currRouteFileName))) {
				const panel = vscode.window.createWebviewPanel(
					'prviewer',
					`${passedInRoute} in ` + currRouteFileName.substring(currRouteFileName.lastIndexOf("/") + 1),
					vscode.ViewColumn.Beside,
					{
						// Enable scripts in the webview
						enableScripts: true
				...
				update_window(document, openFileRoutePanel[currRouteFileName][passedInRoute], true, false, passedInRoute, json_object, undefined);
				return;
			}

			// if we're entering from an already opened hover event or a save event
			if (passedInRoute && (openFileRoutePanel.hasOwnProperty(currRouteFileName)) && 
				openFileRoutePanel[currRouteFileName].hasOwnProperty(passedInRoute)) {
				update_window(document, openFileRoutePanel[currRouteFileName][passedInRoute], true, false, passedInRoute, json_object, undefined);
				return;
			}
			...
			if (!passedInRoute) {
				...
				if (!drawName) {
					vscode.window.showInformationMessage("Please add a route to the file before running the route command.");
					return;
				}

				if (openFileRoutePanel.hasOwnProperty(currRouteFileName) && 
					openFileRoutePanel[currRouteFileName].hasOwnProperty(drawName)) {
						update_window(document, openFileRoutePanel[currRouteFileName][drawName], true, false, drawName, json_object, undefined);
						return;
				}
				const panel = vscode.window.createWebviewPanel(
					'prviewer',
					`${drawName} in ` + currRouteFileName.substring(currRouteFileName.lastIndexOf("/") + 1),
					vscode.ViewColumn.Beside,
					{
						// Enable scripts in the webview
						enableScripts: true
	...
	context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(e => {
		...
		if (vscode.window.activeTextEditor?.document.fileName == undefined ||
			(!(vscode.window.activeTextEditor.document.fileName in openFilePartPanel) &&
			!(vscode.window.activeTextEditor.document.fileName in openFileRoutePanel))) {
			return;
		}
...
function update_window(document: vscode.TextDocument, panel: vscode.WebviewPanel, 
	isRoute: boolean, isPart: boolean, currRouteName: string | undefined, json_object: any,
	currPartName: string | undefined) {
	...
	const switchbox_dict: { [key: string]: object } = getSwitchboxes(json_object, switchbox_dict_keys[0]);
	row_size = get_max_key_value(switchbox_dict, "row") + 1;
	col_size = get_max_key_value(switchbox_dict, "col") + 1;
	
	if (row_size < 1 || col_size < 1) {
		vscode.window.showInformationMessage("Row and / or column size defined to be less than 1.\
	...
	if (isPart) {
		...
		if (typeof(currPartName) === 'undefined') {
			...
			panel.title = `${switchbox_dict_keys[1][0]} in ` + document.fileName.substring(document.fileName.lastIndexOf("/") + 1);
		} else {
			partition_array = get_partition(json_object, [currPartName]);
		}
		const format_check_part = check_partition_array_validity(partition_array, row_size, col_size);
		...
		panel.webview.html = getWebviewContent(colored_grid, region_array, undefined, -1);
		return;
	}
	...
	if (isRoute) {
		...
		if (typeof(currRouteName) === 'undefined') {
			...
			panel.title = `${switchbox_dict_keys[2][0]} in ` + document.fileName.substring(document.fileName.lastIndexOf("/") + 1);
		} else if (currRouteName === 'route_all') {
			var new_keys: Array<string> = [];
			for (var i = 0; i < switchbox_dict_keys[2].length; i++) {
				if (switchbox_dict_keys[2][i].includes("all") || switchbox_dict_keys[2][i].includes("some")) {
					continue;
				} else {
			...
			while ((index = match(routes_to_route, 'route_some')) >= 0) {
				const new_routes = getRoutes(json_object, [routes_to_route[index]]).flat();
		...
		for (var i = 0; i < routes.length; i++) {
			const error_check = check_route_array_validity([routes[i]], row_size, col_size);
			if (error_check) {
				vscode.window.showInformationMessage(error_check);
				return;
			}
		}
		panel.webview.html = getWebviewContent(undefined, [], routes, number_of_routes);
	}
}
...
//   target_elements = ["route01": <some_attributes>, "route02" <some_attributes>]
// Inputs:	
...
// Example:
//   json_file = ["switchbox01": <some_attributes>, "switchbox02": <some_attributes>, 
//               "router01": <some_attributes>]
//   keys = ["switchbox01", "switchbox02"] ->
//   target_elements = ["switchbox01": <some_attributes>, "switchbox02" <some_attributes>]
// Inputs:
//   json_file (Object): parsed JSON file
//   keys (Array<string>): keys to be added to the final dictionary
// Returns:
//   target_elements {[key: string]: object}: dict containing objects of AIE elements
...
//                              e.g. [[<switchbox names>], [<partitions>], [<routes>], [<regions>]]
...
//   [ [[0, 1], ["East", "North"]], [[0, 2], ["East"]], [[1, 1] ["DMA"]], []] - valid
//   [ [[0, 1], ["East", "North"]], [[0, 2], ["North"]], [[1, 1] ["DMA"]], []] - invalid
// Inputs:
...
//                             [ [[<AIE_col #>, <AIE_row #>], ["<direction or DMA>", ...]], ... [] ]
// Outputs:
//   null if valid, string if invalid
function check_route_array_validity(route_array: any, max_row: number, max_col: number) {
	...
	for (var i = 0; i < route_array[0].length; i++) {
		...
		if (route_array[0][i].length !== 2) {
			return "More than 2 elements in a route segment found: " + String(route_array[0][i]) + "\
		...
		if (typeof(route_array[0][i][0][1]) !== 'number' || 
			typeof(route_array[0][i][0][0]) !== 'number') {
				return "Entry in tile [" + String(route_array[0][i][0][1]) + ", " + String(route_array[0][i][0][1]) +"]\
				        is not a number";
			}
		// ensure we're within the bounds of the switchboxes that were provided in the .json
		if (route_array[0][i][0][0] >= max_col || 
			route_array[0][i][0][1] >= max_row ||
			route_array[0][i][0][0] < 0 ||
			route_array[0][i][0][1] < 0) {
				return "Tile coordinates are outside of max range in tile \
				[" + String(route_array[0][i][0][0]) + ", " + String(route_array[0][i][0][1] + "].");
		}
		...
		for (var j = 0; j < route_array[0][i][1].length; j++) {
			if (route_array[0][i][1][j] !== "North" &&
				...
				" + route_array[0][i][0][0] + ", " + route_array[0][i][0][1] + "] is not valid. \
					Please use one of 'North', 'East', 'South', 'West', 'Core', or 'DMA'. ";
			}
			if (route_array[0][i][1][j] === "North") {
				if (route_array[0][i][0][1] === max_row - 1) {
					return "Attempting to draw out of bounds in tile [" + String(route_array[0][i][0][0]) + "\
			...
			if (north > 1 || east > 1 || west > 1 || south > 1 || dma > 1) {
				...
				tile [" + String(route_array[0][i][0][0]) + ", " + String(route_array[0][i][0][1]) + "] \
				 (" + route_array[0][i][1][j] + ").";
			}
		}
	}
...
//   max_row (integer): the number of rows
//   max_col (integer): the number of columns
//   partition_array (object): Array of the partitions in the form [ [Partititon #, [AIE_row, AIE_col]] ]
// Outputs:
//   null if valid, string if invalid
function check_partition_array_validity(partition_array: any, max_row: number, max_col: number) {
	// valid array dimensions
	...
	for (var i = 0; i < partition_array.length; i++) {
		...
		if (partition_array[i].length > 1) {
			...
			for (j; j < partition_array[i].length; j++) {
				...
				if (typeof(partition_array[i][j]) !== 'object' || partition_array[i][j].length !== 2) {
					return "Invalid [x, y] array at partition " + String(i) + ", entry " + String(j);
				}
				if (typeof(partition_array[i][j][0]) !== 'number' || 
				    typeof(partition_array[i][j][1]) !== 'number') {
					return "Arguments in [x, y] tile are not integers at partition " + String(i) + ", entry " + String(j);
				}
				if (partition_array[i][j][0] >= max_row ||
					partition_array[i][j][1] >= max_col ||
					partition_array[i][j][0] < 0 ||
					partition_array[i][j][1] < 0) {
					return "Tile coordinates are outside of max range in partition " + String(i) + ", entry " + String(j);
				}
			}
		}
	}
...
function check_region_array_validity(region_array: Array<number>) {
	...
	if (typeof(region_array[0]) !== 'number' || typeof(region_array[1]) !== 'number') {
		return "Invalid length of region array. Please use the format [<color number>, <# AIEs per row \
			of partition>, <# AIEs per column of partition>]."
	}
	// if (region_array.length === 3) {
	// 	if (typeof(region_array[2]) !== 'number') {
	// 		return "Please ensure that all entries in the array are of an integer value."
	// 	}
	// 	if (row_size % region_array[1] !== 0 || col_size % region_array[2] !== 0) {
	// 		return "Please ensure region works given the grid dimensions."
	// 	}
	// }
	if (region_array.length === 2 && row_size % region_array[0] !== 0 || col_size % region_array[1] !== 0) {
		return "Please ensure region works given the grid dimensions."
	}
}
...
//   max_rows (integer): Number of rows in AIE grid
//   max_cols (integer): Number of columns in AIE grid
// Return:
//   colored_grid (array<array<string>>): Array of strings of colors for AIE tiles, also
// 										  containing the strings of the herd name (optional):
//                                        [[<color>, <name>], ...]
function color_grid(partitions: any, max_rows: number, max_cols: number) {
	...
	for (var i = 0; i < partitions.length; i++) {
		...
		// left tile is [1, 0] in the form <row, col>, as the shim is on the 0th row.
		// rows go up from there. Since the colors will be stored in a 1D grid, the 
		// index's need to be adjusted so that they end up in the right place.
...
function getWebviewContent(colored_grid: Array<Array<string>> | undefined, region: Array<number>, 
						   routes: any, route_number: number) {
	if (route_number > -1) {
    ...
    <head>
      ...
      <body>
		...
		if (${region.length} > 0 && ${route_number} < 0) {
			makeRegions(${region[0]}, ${region[1]}, ${row_size}, ${col_size});
		}
		if (${route_number} > -1) {
			const routes = ${routes};
