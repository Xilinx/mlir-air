...
### `-air-annotate-front-and-back-ops-in-for-pattern`: Annotates ops in for loop body which are at the front and back of the body's dependency graph
...
### `-air-construct-ping-pong-dependency-pattern`: Transform an scf.for loop into ping-pong pattern
...
### `-air-hoist-alloc-in-for-pattern`: Hoist pairs of alloc and dealloc ops out of for loop
...
### `-air-hoist-ops-not-using-ping-pong`: Hoists ops which are not direct users of the target memref
...
### `-air-ping-pong-transform`: Lower to pipelining pattern
...
```
### `-air-place-herds`: Places herds onto a segment.
