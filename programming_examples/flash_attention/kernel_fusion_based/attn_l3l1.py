    ...
    def attention_bf16(q_in, k_in, v_in, gp_out):
        ...
        def launch_body(lx, ly, lsx, lsy, q, k, v, gp):
            ...
            def segment_body(seg_x, seg_y, seg_sx, seg_sy):
                ...
                qk_l2_bufs = [AllocOp(v_l2_t, [], []) for _ in range(NS)]
...
if __name__ == "__main__":
    ...
    sdpa_output = np.zeros((num_heads, lq, dk), dtype=OUTPUT_DATATYPE)
    ...
    if args.compile_mode == "compile-and-run":
        ...
        all_pass = True
        for h in range(num_heads):
            c_h = float(np.corrcoef(npu[h].flatten(), ref[h].flatten())[0, 1])
