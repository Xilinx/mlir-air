...
"""Flash attention with memtile-relayed dataflow on air.api -- NPU2 (AIE2P).
...
* ``window`` masks a sliding window instead of the full causal triangle.
* ``causal_skip`` elides the matmul, mask, softmax and PV for (q_block,
  kv_block) pairs the mask kills outright, rather than computing and discarding
  them. Exact: an all -inf block leaves the online softmax untouched. The
  channel gets stay unconditional, so no DMA is traded -- the herd would
  deadlock on an unbalanced channel.

``causal_skip`` is the one place this needs a branch on a value rather than a
...
def build_module(**kwargs):
    ...
    llms/shared/infra/fa_headfirst.py imports this name and hands the result
    straight to KernelCache.compile_and_cache, which stringifies it into
    air.mlir -- so it must be a module, not the LaunchContext that build_launch
    returns. npu2 is not a parameter here because this file is the npu2
    variant; attn_npu1.py is the other one.
    """
...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        "selective Q capture (NPU2/AIE2P variant)",
    )
...
def main():
    ...
    dv_tile_host = args.dv_tile if args.dv_tile is not None else lkp
