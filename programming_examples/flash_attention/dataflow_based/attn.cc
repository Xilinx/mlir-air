...
void matmul_g_b_bf16(bfloat16 *g_in, bfloat16 *b_in, bfloat16 *out) {
  ...
  // B: [lkp, dv] = [96, 64]
  // Out: [lqp, dv] = [32, 64]
  matmul_vectorized_4x8x4_bf16_bf16<lqp, lkp, dv>(g_in, b_in, out);
}
...
void exp_g_minus_u(bfloat16 *u, bfloat16 *g) {
  ...
  // G <- exp(G)
  aie::vector<bfloat16, 16> exp_val, input_bf16;
  pG = g;
  for (int i = 0; i < num_elems * num_rows; i += VecLen) {
    input_bf16 = aie::load_v<VecLen>(pG);
    exp_val = to_v16bfloat16(getExpBf16(input_bf16));
    aie::store_v(pG, exp_val);
    pG += VecLen;
  }
}
...
void exp_up_minus_u(bfloat16 *up, bfloat16 *u, bfloat16 *r) {
  ...
  // r <- exp(r)
  pr = r;
  aie::vector<bfloat16, VecLen> exp_val, input_bf16;
  for (int i = 0; i < num_elems; i += VecLen) {
    input_bf16 = aie::load_v<VecLen>(pr);
    exp_val = to_v16bfloat16(getExpBf16(input_bf16));
...
void mul_r_gp(bfloat16 *r, bfloat16 *gp) {
  ...
  for (int rowVec = 0; rowVec < num_rows; rowVec += VecLen) {
    ...
    for (int rowVecElem = 0; rowVecElem < VecLen; rowVecElem++) {
      ...
      aie::vector<bfloat16, VecLen> temp;
      for (int32_t i = 0; i < num_elems; i += VecLen) {
        temp = aie::load_v<VecLen>(pGp);
        temp = aie::mul(temp, u_bcast);
...
void sum_g(bfloat16 *g, bfloat16 *s) {
  ...
  for (int rowVec = 0; rowVec < num_rows; rowVec += VecLen) {
    ...
    for (int rowVecElem = 0; rowVecElem < VecLen; rowVecElem++) {
      aie::vector<bfloat16, VecLen> temp;
      bfloat16 max_value = 0.0;
      for (int32_t i = 0; i < num_elems; i += VecLen) {
        temp = aie::load_v<VecLen>(g + i + (rowVec + rowVecElem) * num_elems);
        max_value += aie::reduce_add(temp);
      }
      sVec[rowVecElem] = max_value;
    }
    aie::store_v(ps, sVec);
    ps += VecLen;
  }
