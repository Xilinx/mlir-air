...
"""Vector subtract primitive, on air.api.
...
to say one thing. Only the operator differs.
...
def build_module(n, tile_n, np_dtype_in, vector_size=16):
    ...
    with air.launch(name="vector_sub") as launch:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="run.py",
        description="Builds, runs, and tests the vector_sub example",
    )
    parser.add_argument(
        "-v",
        "--verbose",
