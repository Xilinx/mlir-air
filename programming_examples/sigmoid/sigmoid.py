...
"""Sigmoid, written with the air.api DSL.

Computes out = 0.5 * (tanh(x/2) + 1), the logistic function over a 1-D [N] bf16 vector.
...
def sigmoid_reference(x):
    """Sigmoid reference, evaluated in fp32 and rounded back to the input dtype."""
    xf = x.astype(np.float32)
    return (0.5 * (np.tanh(xf / 2.0) + 1.0)).astype(x.dtype)


def build_sigmoid(n, tile_n, herd_shape=None, vector=None):
    """Build a Sigmoid launch over a 1-D vector of length n."""
    if n % tile_n:
    ...
    with air.launch(name="sigmoid") as launch:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="air.api Sigmoid example")
    ...
    ref = sigmoid_reference(x_np)
    rtol, atol = 1e-1, 5e-2

    module = launch.build(target=args.target)
    print(f"Sigmoid n={args.n} tile_n={args.tile_n} bf16 on {launch.target}")
    runner = XRTRunner(
        verbose=args.verbose,
        omit_while_true_loop=False,
        output_format=args.output_format,
