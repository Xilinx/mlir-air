...
"""Tiled int8 matrix multiplication on air.api.
...
from air.api import i8, i16

import numpy as np

np.random.seed(42)

# The AIE2 matmul intrinsic's operand shape, per architecture. The int8 MAC is
# 4x8x8 on aie2 and 8x8x8 on aie2p.
MMUL_MKN = {"aie2": (4, 8, 8), "aie2p": (8, 8, 8)}

# air.api dtypes, keyed by the numpy dtype the harness works in.
# int8 in, int16 out: the accumulator and the L3 result are wider than the
# operands, which is why dt_in and dt_out are looked up separately below.
DTYPE = {np.int8: i8, np.int16: i16}
...
if __name__ == "__main__":
    ...
    if args.direct_codegen:
        # Both architectures' int8 matmuls accumulate into i32, so unlike the
        # int16 example the accumulator type is not architecture-dependent.
        transform_ir_string = """
    ...
    if args.compile_mode == "compile-and-run":
        ...
        runner = XRTRunner(**runner_kwargs, instance_name="matmul_i8")
        exit(
            runner.run_test(
                mlir_module,
