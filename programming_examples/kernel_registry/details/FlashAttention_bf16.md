...
| 1024×1024 | 64/64 | 12/12 | ✗ | 1 | 2.36–2.61 ms | 1235–1366 | 4.8e-2 | 3.0e-2 | ✅ SmolVLA vision self-attn (SigLIP) — MHA, non-causal, full 8×4 array (`num_heads_per_unroll=2`) |
...
>
> The **1024×1024, 12q/12kv, non-causal, head_dim=64** row is SmolVLA's **vision encoder (SigLIP) self-attention** config — pure MHA (every Q head has its own KV head, no GQA broadcast), **bidirectional / non-causal** (no mask, unlike the decoder backbone's prefix-LM causal attention), seq=1024, head_dim=64 → `dv_chunks=1` (no hang risk). Its **12 heads are even**, so `num_heads_per_unroll=2` divides evenly (`12 % 2 == 0`) and the herd fills the full 8×4 array (`2 heads_per_unroll × 4 q_tiles = 8` columns × `4` cascade rows = 32 tiles). Contrast the SmolVLA decoder backbone's **15-head** attention (odd → `num_heads_per_unroll` must be 1, filling only 4 columns = half the array): being able to run `heads_per_unroll=2` on the even 12-head vision config was verified on real NPU2 to deliver **1235–1366 GFLOP/s vs 864 GFLOP/s** for the same shape forced to `heads_per_unroll=1` half-array — a measured **~1.58× throughput** from filling the chip. Both configs are numerically identical (`mean_rel_L1=4.8e-2`, same `attn_npu2.o`); the win is placement-only.
