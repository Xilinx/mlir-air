...
| 2048×2560×10240 | **fused-cast (m64/k64/n128)** | — | — | 1.0e-2 / — | Gemma3-4B Gate/Up proj | ✅ |
...
> **Gemma3-4B rows — emb=2560, q_dim=8·256=2048 decoupled, kv_dim=1024 (shares the Qwen3-4B 2048×2560×1024 K/V row), hidden=10240=512·20.** Every N is divisible by 4·TILE_N=512, so stock TILE_N=128 HERD_N=4 places, and all four shapes keep the high-precision fused-cast tier at 1.0e-2. Tiles are **inherited** from the nearest same-K / same-N Qwen3-4B neighbour rather than independently swept — the DMA-stride rule `tile_k_l2·N ≤ 1048576` selects the same `tile_k_l2` in each case. Gate/Up at N=10240 needs `tile_k_l2=64` for exactly the reason the Qwen3-4B N=9728 sibling does (128·10240 = 1310720 > limit), and unlike the Qwen2.5 siblings it does **not** have to drop to the low-precision direct path. Gemma3-4B's prefill uses an 8-ELF split (rms_qkv_qknorm_rope / global + sliding-window flash attention / o_norm_res_norm / gate / up / gelu_mul / down_norm_add / lm_head_gemv).
