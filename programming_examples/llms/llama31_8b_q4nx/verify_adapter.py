...
"""Verify adapter for the Q4NX Llama-3.1-8B example (FLM-faithful decode).
...
run through the batched NPU prefill (llama31_8b_q4nx_prefill), whose per-layer
...
Pointed at via `--runner=llama31_8b_q4nx.verify_adapter`.
"""
...
from llama31_8b_q4nx_weights import llama31_8b_config  # noqa: E402
from runners._records import DecodeStepRecord, PrefillRecord  # noqa: E402
from q4nx_decode_8b import FusedDecode8B  # noqa: E402
from llama31_8b_q4nx_inference import PREFILL_MIN_TOKENS  # noqa: E402

# bf16 reference `make verify` compares the NPU against. FastFlowLM's bundle
# declares base_model: meta-llama/Llama-3.1-8B-Instruct, but those repos are
# license-gated; these are faithful ungated re-uploads whose config matches the
# FLM bundle's exactly (including the 3-way eos_token_id list). Same pattern as
# the Gemma3 example's unsloth mirror, and it keeps CI free of a license grant.
MODEL_CHOICES = {
...
Q4NX_MODEL_SOURCE = os.environ.get("Q4NX_MODEL_SOURCE", "FastFlowLM/Llama-3.1-8B-NPU2")
...
# Same crossover the production driver uses (see llama31_8b_q4nx_inference).
...
class FusedDecodeRunner:
    ...
    def _prefiller(self):
        if self._pf is None:
            from llama31_8b_q4nx_prefill import LlamaQ4nxPrefill
...
def build_runner(
    ...
    *,
    ...
    if not npu_attn:
        raise ValueError(
            "llama31_8b_q4nx runs attention on the NPU by design; npu_attn=False "
