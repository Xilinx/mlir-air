...
"""LLAMA-3.1-8B Q4NX Inference on MLIR-AIR (NPU2) — FLM-faithful decode.

The whole decoder layer runs on the AIE array: proj -> RoPE -> flash attention
with an on-device KV cache -> o-proj -> FFN, x32 layers, then the LM head, all in
...
the decode's q4k-cascade layout and cached (see q4nx_decode_8b).

The prompt runs as ONE batched NPU prefill (llama31_8b_q4nx_prefill) whose per-layer
...
Usage:
    python3 llama31_8b_q4nx_inference.py --compile-only     # build prefill ELFs
    python3 llama31_8b_q4nx_inference.py --run-only --n-tokens 32 --prompt "..."
    python3 llama31_8b_q4nx_inference.py --run-only --interactive
"""
...
from q4nx_decode_8b import FusedDecode8B, EOS_IDS  # noqa: E402
...
MODEL_SOURCE_DEFAULT = os.environ.get(
    "Q4NX_MODEL_SOURCE", "FastFlowLM/Llama-3.1-8B-NPU2"
)
...
TOKENIZER_DEFAULT = os.environ.get(
    "Q4NX_TOKENIZER", "NousResearch/Meta-Llama-3.1-8B-Instruct"
)
...
# what keeps a 3B build from silently satisfying an 8B run.
...
def compile_prefill(seq_len: int) -> None:
    ...
    LlamaQ4nxPrefill(seq_len=seq_len, n_layers=32)  # constructing compiles + caches
...
def build_decoder(args) -> FusedDecode8B:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="LLAMA-3.1-8B Q4NX Inference (NPU2, FLM-faithful on-device decode)"
    )
    ...
    if args.profile:
        ...
        print(
            ...
            f"32 layers + LM head on-device, one dispatch/token"
        )
