...
def run_one(cache, lw, layer_idx, pos, seq_len, rng_seed):
    ...
    k_cache_init = (
        rng.standard_normal((N_KV_HEADS, seq_len, HEAD_DIM)) * kv_scale
    ).astype(bfloat16)
    v_cache_init = (
        rng.standard_normal((N_KV_HEADS, seq_len, HEAD_DIM)) * kv_scale
    ).astype(bfloat16)
...
def main():
    ...
    parser.add_argument(
        "--pos", type=int, nargs="+", default=[10, 100, 500, 1000, 1500, 2046]
    )
    ...
    bad_heads = sorted(
        [h for h in range(N_HEADS) if head_min[h] < 0.95], key=lambda h: head_min[h]
    )
    print(
        f"Q-heads with corr < 0.95 in at least one config "
        f"(out of {len(args.layers)*len(args.pos)} configs):"
    )
    for h in bad_heads:
        kv, g = divmod(h, GROUP_SIZE)
        print(
            f"  Q{h:2d} (kv={kv}, g={g}): worst corr = {head_min[h]:.4f}, "
    ...
    if 0 in args.layers:
        ...
        print("=" * 78)
        print(
            f"Summary 4: layer 0, head Q{most_common_outlier} (kv={kv}, g={g}) "
