...
def fused_call(
    ...
    if kc_host is not None:
        ...
        kc_view = np.frombuffer(bos[2].map(), dtype=bfloat16, count=kc.size).reshape(
            kc.shape
        )
        vc_view = np.frombuffer(bos[3].map(), dtype=bfloat16, count=vc.size).reshape(
            vc.shape
        )
...
def main():
    ...
    print(
        f"  Prompt: {args.prompt!r}    N tokens: {args.n_tokens}    "
    ...
    rope_lut_bf16 = generate_rope_lut(
        config=config, seq_len=prefill_seq + args.n_tokens + 1, dtype=bfloat16
    )

    prefill_cache = KernelCache(
        cache_dir="./prefill_kernel_cache", verbose=args.verbose
    )
    ...
    k_cache_sliced = np.zeros(
        (config.n_layers, N_KV_HEADS, fused_seq_len, HEAD_DIM),
        dtype=bfloat16,
    )
    ...
    print(
        f"  done: {time.time() - t0:.2f}s ({config.n_layers} × {B_per_layer[0].nbytes // (1024*1024)} MB)"
    )
    ...
    for token_idx in range(args.n_tokens):
        ...
        for layer_idx in range(config.n_layers):
            ...
            xb = fused_call(
                ...
                v_cache[layer_idx],
                xb_buf,
                kc_host=k_cache[layer_idx],
                vc_host=v_cache[layer_idx],
            )
        ...
        print(
            f"  token {token_idx + 1}: id={next_token}, "
    ...
    if decode_times:
        ...
        print(
            f"  fused_attn (NPU): {f_ms:>5.2f} ms/call * {n_layers} = {f_tot:>5.1f} ms ({100*f_tot/avg_token_ms:>4.1f}%)"
        )
        print(
            f"  o_gemv_ffn (NPU): {o_ms:>5.2f} ms/call * {n_layers} = {o_tot:>5.1f} ms ({100*o_tot/avg_token_ms:>4.1f}%)"
        )
        print(
            f"  pack_xrms (host): {x_ms:>5.2f} ms/call * {n_layers} = {x_tot:>5.1f} ms ({100*x_tot/avg_token_ms:>4.1f}%)"
        )
        print(
            f"  lm_head_gemv (NPU+host stitch): {l_ms:>5.1f} ms ({100*l_ms/avg_token_ms:>4.1f}%)"
        )
        print(
            f"  final RMS-norm (CPU, f32): {r_ms:>5.2f} ms ({100*r_ms/avg_token_ms:>4.1f}%)"
        )
        print(
            f"  unaccounted (other host): {unaccounted:>5.1f} ms ({100*unaccounted/avg_token_ms:>4.1f}%)"
        )
    print()
