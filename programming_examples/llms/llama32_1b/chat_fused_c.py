...
def compile_pos_artifacts(
    prompt_len, n_tokens, seq_len, cache_dir="./fused_pathC_cache"
):
    cache_dir = Path(cache_dir)
    cache_dir.mkdir(parents=True, exist_ok=True)

    print(
        f"[fused-pathC] Compiling {n_tokens} kernels for pos in "
...
class FusedAttnSwapRunner:
    ...
    def add_layer(self, layer_idx, B_bf16, k_cache_init_bf16, v_cache_init_bf16):
        ...
        bo_kc = xrt.bo(
            self.backend.device,
            k_cache_init_bf16.size * 2,
            xrt.bo.host_only,
            self.backend.kernel.group_id(2 + 3),
        )
        bo_vc = xrt.bo(
            self.backend.device,
            v_cache_init_bf16.size * 2,
            xrt.bo.host_only,
            self.backend.kernel.group_id(3 + 3),
        )
    ...
    def run(self, layer_idx, pos, xrms_bf16):
        ...
        xb_view = np.frombuffer(
            self.bo_xb.map(), dtype=bfloat16, count=N_KV_HEADS * GROUP_SIZE * HEAD_DIM
        )
...
def main():
    ...
    print(
        f"  (B BOs: {config.n_layers} * {B.nbytes // (1024*1024)} MB = "
    ...
    if decode_times:
        ...
        print(
            f"Per-token latency:   {avg_token_ms:.1f} ms  ({1000/avg_token_ms:.2f} tok/s)"
        )
        print(
            f"Per-fused-attn-call: {avg_fused_ms:.2f} ms  "
