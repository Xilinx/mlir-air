...
def fmt_corr_table(name, overall, ct, et):
    ...
    print(
        f"  per-head max_abs_err min/mean/max: "
        f"{et.min():.4f} / {et.mean():.4f} / {et.max():.4f}"
    )
    worst = np.unravel_index(np.argmin(ct), ct.shape)
    print(
        f"  worst head: Q{worst[0]*GROUP_SIZE + worst[1]} "
...
def main():
    ...
    rope_lut_bf16 = generate_rope_lut(
        config=config, seq_len=prompt_len + 4, dtype=bfloat16
    )
    ...
    print(
        f"  x_decode magnitude: max(|x|)={float(np.max(np.abs(x_decode.astype(np.float32)))):.4f}, "
        f"mean(|x|)={float(np.mean(np.abs(x_decode.astype(np.float32)))):.4f}"
    )
    print(
        f"  k_cache[L{layer_idx}] (prefill) at slot 0: "
        f"max(|k|)={float(np.max(np.abs(k_cache[layer_idx, :, 0, :].astype(np.float32)))):.4f}"
    )
    print(
        f"  k_cache[L{layer_idx}] (prefill) at slot prompt_len-1: "
        f"max(|k|)={float(np.max(np.abs(k_cache[layer_idx, :, prompt_len-1, :].astype(np.float32)))):.4f}"
    )
    print(
        f"  k_cache[L{layer_idx}] (prefill) at slot prompt_len (uninit): "
