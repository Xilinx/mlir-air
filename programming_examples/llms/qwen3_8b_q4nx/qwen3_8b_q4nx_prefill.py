...
#
# Qwen3-8B Q4NX prefill in mlir-air.
#
...
#
# This is the 8B analog of llama32_3b_q4nx_prefill.py. qwen3_4b is the same
# architecture (Qwen3 QK-norm, no bias, eps=1e-6, head_dim=128, single-theta
# RoPE 1e6) and its builders are config-driven, so only the dims change:
#
...
#
# At 8B q_dim == emb_dim == 4096, so the DECOUPLED O proj degenerates to a square
# GEMM; the builder is parameterized on (q_dim, emb_dim) and needs no change.
# All five GEMM shapes resolve in the kernel registry (fused-cast high-prec):
# Q/O 2048x4096x4096 tk_l2=512, K/V x1024 tk_l2=256, Gate/Up x12288 tk_l2=64
# (the f32-out B-tile DMA stride cap tile_k_l2*N <= 1048576 leaves only 64 legal
# at N=12288), Down 2048x12288x4096 tk_l2=512.
#
...
_LM_N_PART = 16384
_LM_N_PARTITIONS = 10
# K=4096 breaks both default GEMV budgets: the L2 A stage needs
...
class Qwen3Q4nxPrefill:
    """AIR realization of the Q4NX causal-LM prefill interface (Qwen3-8B)."""
    ...
    def load_weights(self, model=None):
        ...
        from qwen3_8b_q4nx_weights import load_q4nx_weights
    ...
    def _preload_lm_head_gemv(self):
        ...
        into resident BOs once (static; skipped thereafter). Qwen3-8B does NOT
        tie embeddings, so this is its own Q4NX-sourced matrix, not the embed."""
        self._lm_parts = []
...
def _main():
    ap = argparse.ArgumentParser(description="Qwen3-8B Q4NX prefill on NPU2")
    ap.add_argument(
        "--compile-only",
        action="store_true",
