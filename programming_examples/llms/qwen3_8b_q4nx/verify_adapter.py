...
"""Verify adapter for the Q4NX Qwen3-8B example (FLM-faithful prefill + decode).

Drives the SAME on-device path `make run` uses: the batched AIR prefill for the
prompt, then the fused_decode superkernel (36 layers + the untied LM head in one
dispatch, attention + KV cache on the AIE array) for each new token.

REFERENCE. FastFlowLM/Qwen3-8B-NPU2's model card declares `base_model:
Qwen/Qwen3-8B`. Qwen3 ships a single ungated checkpoint -- there is no separate
base/instruct split -- so both MODEL_CHOICES keys map to it, and the gate
compares NPU-q4nx against that HF bf16 forward.
...
Pointed at via `--runner=qwen3_8b_q4nx.verify_adapter`.
"""
...
Q4NX_MODEL_SOURCE = os.environ.get("Q4NX_MODEL_SOURCE", "FastFlowLM/Qwen3-8B-NPU2")

# Padded prefill length. The Qwen3-8B GEMM registry shapes exist at M=2048, so the
...
def build_runner(
    ...
    *,
    ...
    """Compile/load the Q4NX prefill + the fused Qwen3-8B decode, return an NpuRunner."""
    return NpuRunner(tokenizer=tokenizer, lite_mode=lite_mode)


class NpuRunner:
    """Adapter over the Q4NX batched prefill + one-xclbin fused Qwen3-8B decode.
    ...
    (36 layers + the untied LM head) through `FusedDecoder`. Mirrors the loop in
    `qwen3_8b_q4nx_inference.generate`."""
    ...
    def prefill(self, prompt_tokens: np.ndarray) -> PrefillRecord:
        ...
        if len(ids) > self.attn_maxl:
            raise SystemExit(
                f"prompt is {len(ids)} tokens but the fused Qwen3-8B decode KV cache "
