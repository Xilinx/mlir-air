...
"""qwen3_8b_q4nx full inference -- FLM-faithful Qwen3-8B prefill + decode.

Reproduces FastFlowLM's Qwen3-8B mechanism end to end on the NPU:

  prefill: the batched AIR prefill (qwen3_8b_q4nx_prefill.Qwen3Q4nxPrefill) -- a
  ...
  decode: the shared fused_decode engine (DECODE_MODEL=qwen3-8b) -- 36 decoder
...
(qwen3_8b_q4nx_weights.forward_prompt) -- the oracle the prefill is checked
...
Run:
  python3 qwen3_8b_q4nx_inference.py                 # Paris gate (greedy, PARIS_GREEDY)
  python3 qwen3_8b_q4nx_inference.py --prompt "The capital of France is" --n-tokens 12
  python3 qwen3_8b_q4nx_inference.py --numpy-prefill # numpy-oracle prefill instead
"""
...
# (qwen3_8b_q4nx_weights resolves all three).
...
def _ensure_requant_cache(fd, model):
    ...
    rc = os.environ.get("Q4NX_QWEN3_8B_DECODE_NPZ")
    if rc and os.path.exists(rc):
        return rc
    import qwen3_8b_q4nx_requant as rq
    ...
    os.environ["Q4NX_QWEN3_8B_DECODE_NPZ"] = rc
    return rc


# Qwen3-8B decode xclbins (decode_L<N>) live in this example's own artifact dir so they
...
_DECODE_DIR = Path(os.environ.get("Q4NX_QWEN3_8B_DECODE_DIR", str(_HERE)))
...
class FusedDecoder:
    """One-xclbin Qwen3-8B fused decode. A SINGLE decode xclbin built at ATTN_MAXL serves
    ...
    def __init__(self, model=MODEL_DEFAULT, max_L=None):
        ...
        _z = np.load(os.environ["Q4NX_QWEN3_8B_DECODE_NPZ"])
        ...
        print(
            f"[decode] qwen3-8b ONE xclbin: ATTN_MAXL={self.ATTN_MAXL}, serves "
        ...
        if self.elf_mode:
            ...
            # Same >4 GiB weight split as the xclbin path: one BO is addressable
            # only over a 4 GiB span. Group 0 stays on the weight argument and the
            # rest are appended after kvc, so the binding positions match what the
            # ELF was built for.
            ...
            if self._wsplit:
                ...
                self.w_bos = [xrt.ext.bo(self.dev, p.size * 2) for p in _parts]
                for _bo, _p in zip(self.w_bos, _parts):
                    _bo.write(np.ascontiguousarray(_p).view(np.int16), 0)
                    _bo.sync(TO)
            ...
            self._elfdec.bind(
                ...
                + tuple(self.w_bos[1:] if self._wsplit else ())
            )
