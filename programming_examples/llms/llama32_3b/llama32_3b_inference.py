...
def _preload_decode_weights(decode_cache, weights, config):
    ...
    print("  Pre-loading decode weights into per-layer BOs (rms_gemv_rope + LM-head)...")
    ...
    for li in range(config.n_layers):
        ...
        decode_cache.load_and_run(
            "o_gemv", _decode_mod._gemv_backend(False, "o_gemv"),
        ...
        decode_cache.load_and_run(
            "gate_gemv", _decode_mod._gemv_backend(False, "gate_gemv"),
        ...
        decode_cache.load_and_run(
            "up_gemv", _decode_mod._gemv_backend(False, "up_gemv"),
        ...
        # static {0}.
        decode_cache.load_and_run(
            "down_gemv", _decode_mod._gemv_backend(False, "down_gemv", omit_pingpong=True),
...
def run_npu_prefill(
    token_ids, weights, config, prefill_cache, decode_cache, rope_lut_bf16,
    max_seq, tokenizer, cpu_attn=True, profile=False, quiet=False,
):
    ...
    for layer_idx in range(config.n_layers):
        ...
        x_bf16, inter = run_transformer_block(
            x_bf16, weights.layers[layer_idx], rope_lut_bf16, config,
            prefill_cache, layer_idx=layer_idx, cpu_attn=cpu_attn, verbose=profile,
        )
    ...
    with prefill_cache.profiler.time_cpu("final_rms_norm"):
        ...
        last_normed = rms_norm(last_hidden, weights.final_norm).flatten().astype(bfloat16)
...
def run_npu_decode_step(
    ...
    for layer_idx in range(config.n_layers):
        ...
        x = run_decode_block(
            ...
            k_cache[layer_idx], v_cache[layer_idx], current_pos, rope_lut_bf16,
        )
        decode_cache.profiler.end_layer(layer_idx, t0)

    with decode_cache.profiler.time_cpu("final_rms_norm"):
        x_normed = rms_norm(x.astype(np.float32).reshape(1, config.emb_dim), weights.final_norm)
...
def generate(
    prompt_tokens, weights, config, prefill_cache, decode_cache, rope_lut_bf16,
    tokenizer, n_tokens=10, profile=False, cpu_attn=True, on_token=None, ttft_start=None,
):
...
if __name__ == "__main__":
    ...
    if args.interactive:
        ...
        generated, plen = run_once(
            session, args.prompt, n_tokens=args.n_tokens, profile=args.profile, cpu_attn=args.cpu_attn
        )
