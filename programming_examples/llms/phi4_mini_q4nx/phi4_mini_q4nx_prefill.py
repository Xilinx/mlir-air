...
#
# Phi-4-mini Q4NX prefill in mlir-air.
#
...
_LM_N_PART = 16384
_LM_N_PARTITIONS = 13  # 13*16384 = 212992 >= 200064
...
class LlamaQ4nxPrefill:
    """AIR realization of the Q4NX causal-LM prefill interface (Phi-4-mini)."""
    ...
    def __init__(self, seq_len=2048, n_layers=32, cache_dir=None, verbose=False):
        ...
        # PARTIAL ROTARY: the rope launch DMAs the LUT row at the same offset and
        # width as the data row, so the LUT is head_dim wide with the trailing
        # (head_dim - ROPE_DIM) columns zero -- rope_partial never reads them.
        self._rope_lut = _bf(
            generate_rope_lut(
                config=self.config, seq_len=seq_len, width=self.config.head_dim
            )
        )
        ...
        if not {"rms_gemms_rope", "o_ffn", "flash_attn"}.issubset(cached):
            compile_all_kernels(
                ...
                seq_len,
                cpu_attn=False,
                # 96 of 128: RoPE covers the leading 3/4, the tail passes through.
                rope_dim=ROPE_DIM,
            )
        else:
            print(
    ...
    def load_weights(self, model=None):
        ...
        from phi4_mini_q4nx_weights import load_q4nx_weights
...
def _main():
    ap = argparse.ArgumentParser(description="Phi-4-mini Q4NX prefill on NPU2")
    ap.add_argument(
        "--compile-only",
        action="store_true",
