...
"""PHI-4-MINI Q4NX Inference on MLIR-AIR (NPU2) — FLM-faithful decode.
...
the decode's q4k-cascade layout and cached (see q4nx_decode_phi4).

The prompt runs as ONE batched NPU prefill (phi4_mini_q4nx_prefill) whose per-layer
...
Usage:
    python3 phi4_mini_q4nx_inference.py --compile-only     # build prefill ELFs
    python3 phi4_mini_q4nx_inference.py --run-only --n-tokens 32 --prompt "..."
    python3 phi4_mini_q4nx_inference.py --run-only --interactive
"""
...
from q4nx_decode_phi4 import FusedDecodePhi4, EOS_IDS  # noqa: E402
...
TOKENIZER_DEFAULT = os.environ.get("Q4NX_TOKENIZER", "microsoft/Phi-4-mini-instruct")
...
# what keeps another model's build from silently satisfying a Phi-4 run.
...
def build_decoder(args) -> FusedDecodePhi4:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="PHI-4-MINI Q4NX Inference (NPU2, FLM-faithful on-device decode)"
    )
