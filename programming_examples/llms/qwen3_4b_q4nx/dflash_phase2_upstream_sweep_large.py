...
def _load_upstream(name):
    spec = importlib.util.spec_from_file_location(f"dflash_upstream_{name}", str(_UPSTREAM / f"{name}.py"))
...
def main():
    ...
    print(f"[sweep_large] loading draft {DRAFT_ID} via upstream model.py...", flush=True)
    ...
    for ds_name in DATASETS_TO_RUN:
        ...
        print(f"[sweep_large] {ds_name}: {len(sample)} prompts selected (seed 42, matches upstream benchmark.py)", flush=True)
    ...
    print("\n" + "=" * 70)
    print(f"[sweep_large] SUMMARY ({N_PER_DATASET} real prompts/dataset, thinking disabled, block16, greedy)")
    print("=" * 70)
    grand = []
    for ds_name, lens in by_dataset.items():
        ...
        print(f"  {ds_name:<12} n_blocks={n:>4}  mean={mean:.2f}/16 ({100*mean/16:.1f}%)")
    if grand:
        n = len(grand)
        mean = sum(grand) / n
        print("-" * 70)
        print(f"  {'OVERALL':<12} n_blocks={n:>4}  mean={mean:.2f}/16 ({100*mean/16:.1f}%)")
    return 0
