...
class FusedDecoder:
    ...
    def __init__(self, model=MODEL_DEFAULT, max_L=None, staircase=False):
        ...
        if self.elf_mode:
            ...
            # Same weight-split decision as the xclbin path (one BO is addressable
            # only over a 4 GiB span). Inactive at this size, but _wsplit is read
            # again after this block, so it has to be set either way.
