...
#
# Qwen3-4B Q4NX prefill in mlir-air -- the DFlash target model
# (docs/DFlashFeasibility.md).
#
# Runs the prefill MECHANISM on the AMD NPU2: Q4NX weights, host dequant
# Q4NX->bf16 at load, then op-by-op bf16 GEMM / RMSNorm / QK-norm / RoPE /
# SwiGLU / causal-GQA-flash-attention all ON THE NPU with RESIDENT weight BOs,
# and the final LM head as an on-device GEMV. The transformer block runs through
# the qwen3_4b builders (rms_qkv_qknorm_rope + o_res_norm + gate + up + swiglu +
# down_add) plus head-first flash attention (head_dim=128), driven by
# KernelCache.load_and_run with per-layer resident BOs (weights written once).
#
# This is qwen3_8b_q4nx_prefill.py at Qwen3-4B's dimensions (same topology,
# same builders -- only the config differs) plus ONE real delta: Qwen3-4B TIES
# its embedding and lm_head, so there is no separate Q4NX lm_head tensor and
# the on-device LM head GEMV partitions read the embedding table instead.
#
#     qwen3_4b : emb=2560, q_dim=4096, kv_dim=1024, hidden=9728, 36L, TIED
#     qwen3_8b : emb=4096, q_dim=4096, kv_dim=1024, hidden=12288, 36L, untied
#
# It captures per-layer roped-K + raw-V so the fused decode can be seeded with a
# warm KV cache -- prompt tokens then cost one batched prefill instead of one
# full decode dispatch each.
#
...
VOCAB = 151936
# "The capital of France is" under the Qwen3 tokenizer (no BOS).
...
_LM_N_PART = 16384
_LM_N_PARTITIONS = 10
# K=4096 (q_dim, unaffected by D shrinking to 2560) breaks both default GEMV
# budgets the same way it does for 8B: the L2 A stage needs
# herd_m*tile_m*(K+1)*2 <= 512 KiB (so herd_m*tile_m <= 63, vs the default 64),
# and m_input=4 puts the ping-pong L1 A tiles at 2*32 KiB = all of L1. herd_m
# must stay 8 -- 4 builds but hangs the dispatch -- so tile_m halves instead,
# which means mv.o has to be rebuilt at DIM_M_OUTPUT=tile_m.
#
# NOTE: the LM head GEMV here contracts over D=2560 (the embedding dim), not
# the q_dim=4096 the comment above describes for 8B's untied lm_head -- Qwen3-4B
# is tied, so the head IS the [VOCAB, D] embedding table. D=2560 fits the
# default GEMV budget (herd_m*tile_m <= 63 needs no change at K=2560), but the
# same _LM_HERD_M/_LM_TILE_M/_LM_M_INPUT are kept for parity with 8B's mv.o
# build unless profiling says otherwise.
_LM_HERD_M = 8
_LM_TILE_M = 4
_LM_M_INPUT = 2
...
class Qwen3Q4nxPrefill:
    """AIR realization of the Q4NX causal-LM prefill interface (Qwen3-4B)."""
    ...
    def __init__(self, seq_len=2048, n_layers=36, cache_dir=None, verbose=False):
        ...
        from qwen3_4b_q4nx_weights import qwen3_4b_q4nx_config
        ...
        self.config = qwen3_4b_q4nx_config(n_layers=n_layers)
        ...
        )  # half-split, theta=1e6
        self._lm_backend = dict(LM_GEMV_BACKEND)

        # Compile (or reuse cached) the ELFs: the five qwen3_4b block stages plus
        # flash_attn, and the 10-partition lm_head GEMV. Q4NX_FORCE_COMPILE=1
        # forces recompile; otherwise a valid manifest is reused.
        force = os.environ.get("Q4NX_FORCE_COMPILE") == "1"
        if not force:
            self.cache.load_manifest()
        cached = set() if force else set(self.cache.artifacts)
        need = {"rms_qkv_qknorm_rope", "o_res_norm", "gate", "up", "swiglu", "down_add"}
    ...
    def load_weights(self, model=None):
        ...
        from qwen3_4b_q4nx_weights import load_q4nx_weights
        from qwen3_4b_prefill import preload_prefill_weights
        ...
        self._lm_head = w.lm_head  # TIED: the same array as w.embed_table
    ...
    def _preload_lm_head_gemv(self):
        """Build the 10 padded bf16 lm_head partitions [16384, D] and write them
        into resident BOs once (static; skipped thereafter). Qwen3-4B TIES
        embeddings, so this is the embedding table dequanted as bf16, not a
        separate Q4NX-sourced matrix."""
        self._lm_parts = []
    ...
    def _run_layer(self, x, k, N):
        """One transformer block fully on-device via the qwen3_4b builders
        (rms+qkv+qk-norm+rope, flash attention, o+residual+rms, gate/up/SwiGLU,
        down+add). Captures roped-K + raw-V into the KV cache."""
        from qwen3_4b_prefill import run_transformer_block_qwen3
...
def _main():
    ap = argparse.ArgumentParser(description="Qwen3-4B Q4NX prefill on NPU2")
    ap.add_argument(
        "--compile-only",
        action="store_true",
