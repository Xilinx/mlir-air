...
def main():
    ...
    m._cuda_time = (
        lambda: _time.perf_counter()
    )  # CPU-only: skip torch.cuda.synchronize()

    print(
        f"[upstream_check] loading target {TARGET_ID} (bf16, CPU, sdpa)...", flush=True
    )
    ...
    print(
        f"[upstream_check] loading draft {DRAFT_ID} via upstream model.py's DFlashDraftModel...",
        flush=True,
    )
    ...
    def run(label, prompt_ids, n_tokens):
        ...
        print(
            f"[upstream_check] {label}: {len(lens)} blocks, accepted lengths (produced tokens/block) = {lens}"
        )
        print(
            f"[upstream_check] {label}: mean produced/block = {mean:.2f} of 16 ({100*mean/16:.1f}%)"
        )
        return lens

    print(
        "\n=== 1) raw sentence completion (matches this session's Phase 2 prompts) ===",
        flush=True,
    )
    ...
    print(
        "\n=== 2) chat-templated GSM8K-style task (matches dflash/benchmark.py's real eval) ===",
        flush=True,
    )
    gsm8k_question = (
