...
# The NPU weight bundle: an HF repo id by default, like every other q4nx
# example. Override with --bundle or Q4NX_MODEL_SOURCE.
BUNDLE = os.environ.get("Q4NX_MODEL_SOURCE", "FastFlowLM/Gemma4-E2B-IT-NPU2")

# FastFlowLM's golden activations. Unlike the bundle this is NOT published: it is
# an artifact of an FLM source tree (decoding/references/gemma4_e2b_ref.safetensors,
# regenerated by their gen_gemma4_ref.py). So this gate is a local development
# tool, not something CI can run -- the NPU gate CI does run is the per-layer
# device check in validate_layer_npu.py. FLM_REFERENCE points at it.
...
def main():
    ...
    if not a.ref:
        raise SystemExit(
            "This gate compares against FastFlowLM's own golden activations, which "
            "are an artifact of an FLM source tree and are not published. Point "
            "--ref (or FLM_REFERENCE) at "
            "FLM_Xclbin/Gemma4/decoding/references/gemma4_e2b_ref.safetensors.\n"
            "The gate that needs no FLM tree is the on-device per-layer check: "
            "`make layer-gate` (validate_layer_npu.py)."
        )
    ref = load_ref(a.ref)
