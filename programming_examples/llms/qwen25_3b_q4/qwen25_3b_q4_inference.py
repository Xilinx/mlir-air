...
"""Qwen2.5-3B Q4NX end-to-end inference on NPU2 (MLIR-AIR).

Two halves, both on device:
  prefill: the op-by-op AIR prefill (`Qwen25Q4Prefill`) -- full logits for the
    first token plus the per-layer roped-K / raw-V that seed the decode.
  decode: the shared fused_decode engine (DECODE_MODEL=qwen2.5-3b) -- 36 decoder
    layers plus the tied LM head in ONE dispatch, attention and KV cache on the
    AIE array. One xclbin built at ATTN_MAXL serves every context length via a
    per-token RTP-L + KV-append instruction patch.

  python3 qwen25_3b_q4_inference.py                  # Paris gate (greedy, PARIS_GREEDY)
  python3 qwen25_3b_q4_inference.py --prompt "..."   # chat-templated one-shot
  python3 qwen25_3b_q4_inference.py --interactive    # chat REPL
  python3 qwen25_3b_q4_inference.py --numpy-prefill  # numpy-oracle prefill instead
"""
...
# Weight source: FastFlowLM's pre-quantized Q4NX bundle by default. A local dir
# containing model.q4nx, a direct model.q4nx path, or a full-precision HF
# checkpoint (quantized onto the Q4NX grid at load) all work --
# qwen25_3b_q4_weights.open_weight_source resolves all four.
...
PARIS_GREEDY = [12095, 13, 12095, 374, 279, 7772, 3283, 304, 9625, 13]
EOS_IDS = (151643, 151645)  # <|endoftext|>, <|im_end|>
_Q4NX_CACHE = os.path.expanduser("~/.cache/q4nx_qwen25_3b")


def _ensure_requant_cache(fd, model):
    """Return the decode q4k-cascade cache path, building it from the weight source on
    first use (one-time pack of 36 layers + the tied lm-head).

    Keyed by (VOCAB_I2, W_DUAL_CHAN): the lm-head pack depends on the vocab chunk size
    and W_DUAL_CHAN reorders the cascade, so a warm cache built with either one
    different would feed the xclbin the wrong blocks rather than fail."""
    rc = os.environ.get("Q4NX_QWEN25_3B_DECODE_NPZ")
    ...
    os.environ["Q4NX_QWEN25_3B_DECODE_NPZ"] = rc
    return rc


# Qwen2.5-3B decode xclbins (decode_L<N>) live in this example's own artifact dir so they
...
_DECODE_DIR = Path(os.environ.get("Q4NX_QWEN25_3B_DECODE_DIR", str(_HERE)))
...
class FusedDecoder:
    """One-xclbin Qwen2.5-3B fused decode. A SINGLE decode xclbin built at ATTN_MAXL serves
    ...
    def __init__(self, model=MODEL_DEFAULT, max_L=None, staircase=False):
        ...
        spec = importlib.util.spec_from_file_location(
            "fu_qwen25_3b", str(_DEC / "fused_decode.py")
        )
        ...
        _z = np.load(os.environ["Q4NX_QWEN25_3B_DECODE_NPZ"])
        ...
        print(
            f"[decode] qwen2.5-3b ONE xclbin: ATTN_MAXL={self.ATTN_MAXL}, serves "
            f"L in [1,{self.maxL}]; {self.UNI_DEC} layers + lm-head/dispatch",
            flush=True,
        )

        # ONE xclbin + ONE self-contained BO set (weight BO uploaded once). 36 layers of
    ...
    def seed_kv(self, fk, fv, P):
        """Place the prefill K/V (fk/fv: [UNI_DEC,P,DK_TOT_A]) into the device KV cache
...
def _build_prefiller(model, seq_len=None):
    ...
    This is the model-load cost -- the ELF cache plus the Q4NX host dequant and the
    one-time write of every weight into its BO. Split out from the per-turn prefill
    so an interactive session pays it once."""
    import time
...
def _kv_stack(pf, n_layers):
    """(Kc, Vc) [n_layers, P, DK] from the prefill's per-layer KV views."""
    import numpy as np

    K = np.stack([np.asarray(pf.kv_view(L)[0], np.float32) for L in range(n_layers)])
    V = np.stack([np.asarray(pf.kv_view(L)[1], np.float32) for L in range(n_layers)])
    return K, V


def _prefill_turn(pf, prompt):
    """One AIR prefill -> (Kc, Vc, first_token, ttft_s).

    Kc/Vc are [NUM_LAYERS, P, DK] roped-K / raw-V, the layout FusedDecoder.seed_kv
    consumes. ttft_s times the prefill only; the model load is reported separately."""
    import time
    import qwen25_3b_q4_weights as gw
    ...
    Kc, Vc = _kv_stack(pf, gw.NUM_LAYERS)
...
# The bundle carries no tokenizer; detok/chat-templating come from the upstream
# checkpoint the bundle was quantized from.
TOKENIZER_REPO = os.environ.get("QWEN_Q4_TOKENIZER", "Qwen/Qwen2.5-3B-Instruct")
...
def _detok(ids):
    try:
        ...
        return AutoTokenizer.from_pretrained(TOKENIZER_REPO).decode(ids)
...
def repl(model=MODEL_DEFAULT, n_tokens=64):
    ...
    while True:
        ...
        # from this turn's prefill alone.
        pf.clear_context()
...
def main():
    ap = argparse.ArgumentParser(description="qwen25_3b_q4 full inference (decode)")
    ...
    if args.prompt_ids:
        ...
        prompt = _format_prompt(
            AutoTokenizer.from_pretrained(TOKENIZER_REPO), args.prompt
        )
    else:
        prompt = PARIS_PROMPT
