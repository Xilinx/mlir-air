...
if RMS_MEMTILE_REFEED == 3 and N_EXTRA and not _RF_EXTRA_OK:
    raise SystemExit(
        ...
        f"\n"
        f"  YOU ALMOST CERTAINLY DO NOT NEED THIS. DFlash's pre-pass waves\n"
        f"  ride the DRAFTER's template now, which is mode 0 and carries them\n"
        f"  unchanged -- so the target is free to be a mode-3 build with no\n"
        f"  wave table at all. That is `--prepass draft`, the loop's default;\n"
        f"  see llms/qwen3_4b_q4nx/README.md section 3b. This refusal is only\n"
        f"  in the way of putting them on a mode-3 TARGET, which is a closed\n"
        f"  route (BZeroPlan, 'The shape that fails, named exactly').\n"
