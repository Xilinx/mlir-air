...
// GLU_SLICE is DERIVED, not a model fact: fused_decode.py fixes it from the
...
static_assert(GLU_SLICE == GLU_SLICE_EXPECTED,
              "models/<model>.h GLU_SLICE disagrees with the value "
              "fused_decode.py derives for this model");
#endif
