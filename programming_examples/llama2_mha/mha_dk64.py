...
def build_module(
    ...
    for func in [
        ...
        func.attributes["link_with"] = StringAttr.get("mha_dk64.o")
    ...
    def mha_bf16(arg0, arg1, arg2, arg3, arg4):
        ...
        def launch_body(
            ...
            def segment_body():
                ...
                herd_body_0.attributes["link_with"] = StringAttr.get("mha_dk64.o")
...
if __name__ == "__main__":
    ...
    # Realistic uniform random inputs; float32 reference with bf16 round-trips
    # to mirror in-kernel storage precision. Same pattern as prefill attn_npu2.py.
    rng = np.random.default_rng(42)
    val_range = 4.0
    input_a = rng.uniform(0, val_range, (QKV_COUNT, args.k)).astype(INPUT_DATATYPE)
    input_b = rng.uniform(0, val_range, (QKV_COUNT, args.k, args.n)).astype(
        INPUT_DATATYPE
    )

    Af = input_a.astype(np.float32)
    Bf = input_b.astype(np.float32)
    output_vm = np.zeros((QKV_COUNT, args.n), dtype=np.float32)
    ...
    output_c = output_vm.reshape(-1)

    # RoPE on Q (offset 0) and K (offset args.n).
    for s in range(0, args.n, 2):
        ...
        v0 = output_c[s + args.n]
        v1 = output_c[s + args.n + 1]
        output_c[s + args.n] = v0 * fcr - v1 * fci
        output_c[s + args.n + 1] = v0 * fci + v1 * fcr
    output_c = output_c.astype(OUTPUT_DATATYPE).astype(np.float32)

    output_q = output_c[: args.n]
    K_new_f = output_c[args.n : 2 * args.n]
    V_new_f = output_c[2 * args.n :]
    ...
    # Attention in float32, bf16 at the end.
    ...
    for t in range(args.pos + 1):
        scores[t] = float(np.dot(output_q, Kc_f[t])) * inv_sqrt_n
    scores -= scores.max()
    P = np.exp(scores)
    P /= P.sum()
    ...
    for t in range(args.pos + 1):
        xb_f += P[t] * Vc_f[t]
    output_xb = xb_f.astype(OUTPUT_DATATYPE)
