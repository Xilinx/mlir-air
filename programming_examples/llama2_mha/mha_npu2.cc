...
//
// AIE2P (NPU2) port of mha.cc kernels. Same extern "C" symbol names as
// mha.cc so mha.py can switch by changing link_with from "mha.o" to
// "mha_npu2.o" with no other Python changes.
//
...
//------------------------------------------------------------------------------
// freq_pos: out[i] = pos / 10000^(2i/dk). dk=48 => 24 entries.
// Frequency table precomputed for dk=48 (24 entries), padded to 32 with zeros
// so n=16 vector loads stay in-bounds.
//------------------------------------------------------------------------------
alignas(aie::vector_decl_align) static const bfloat16 freq_table_dk48[32] = {
    // 1.0 / 10000^(2*i / 48), i = 0..23
...
// 24-elem outer interface, 32-elem internal padded buffer (n=16 native).
void freq_pos_bf16_24_8(int pos, bfloat16 *outputs) {
  alignas(aie::vector_decl_align) bfloat16 work[32];
  freq_pos_bf16<32, VEC>(pos, freq_table_dk48, work);
  for (unsigned j = 0; j < HEAD_SIZE_BY_TWO; j++)
    outputs[j] = work[j];
}

void sinf_bf16_24_8(const bfloat16 *inputs, bfloat16 *outputs) {
  alignas(aie::vector_decl_align) bfloat16 in_pad[32], out_pad[32];
  for (unsigned j = 0; j < HEAD_SIZE_BY_TWO; j++)
    in_pad[j] = inputs[j];
  for (unsigned j = HEAD_SIZE_BY_TWO; j < 32; j++)
    in_pad[j] = (bfloat16)0.0f;
  sinf_cosf_poly_bf16<32, VEC, true>(in_pad, out_pad);
  for (unsigned j = 0; j < HEAD_SIZE_BY_TWO; j++)
    outputs[j] = out_pad[j];
}

void cosf_bf16_24_8(const bfloat16 *inputs, bfloat16 *outputs) {
  alignas(aie::vector_decl_align) bfloat16 in_pad[32], out_pad[32];
  for (unsigned j = 0; j < HEAD_SIZE_BY_TWO; j++)
    in_pad[j] = inputs[j];
  for (unsigned j = HEAD_SIZE_BY_TWO; j < 32; j++)
    in_pad[j] = (bfloat16)0.0f;
  sinf_cosf_poly_bf16<32, VEC, false>(in_pad, out_pad);
  for (unsigned j = 0; j < HEAD_SIZE_BY_TWO; j++)
    outputs[j] = out_pad[j];
}

void shuffle_apply_rope_bf16_48(int offset, const bfloat16 *fcr,
                                const bfloat16 *fci, bfloat16 *outputs) {
  // fcr/fci come in as 24-elem buffers; pad to 32 for native n=16 ops.
  alignas(aie::vector_decl_align) bfloat16 fcr_pad[32], fci_pad[32];
  for (unsigned j = 0; j < HEAD_SIZE_BY_TWO; j++) {
    fcr_pad[j] = fcr[j];
    fci_pad[j] = fci[j];
  }
  for (unsigned j = HEAD_SIZE_BY_TWO; j < 32; j++) {
    fcr_pad[j] = (bfloat16)0.0f;
    fci_pad[j] = (bfloat16)0.0f;
  }
  shuffle_apply_rope_padded<HEAD_SIZE>(offset, fcr_pad, fci_pad, outputs);
}

void vector_copy(int offset, const bfloat16 *inputs, bfloat16 *outputs) {
  bfloat16 *out = outputs + offset;
  for (unsigned j = 0; j < 48; j += 16) {
