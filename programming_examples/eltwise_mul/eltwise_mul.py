...
"""Element-wise Multiply, written with the air.api DSL.
...
Concepts:
  ...
  - whole-tile elementwise assignment (c[:] = a[:] * b[:]), which lowers to a
    vector.transfer_read / arith.mulf / transfer_write loop
"""
...
def build_eltwise_mul(shape, dtype=bf16, tile=None, herd_shape=None, vector=None):
    """Build an elementwise-multiply launch over a 1-D or 2-D shape."""
    rank = len(shape)
    if rank not in (1, 2):
    ...
    def tile_body(h, coords):
        """One tile: stage both operands into L1, multiply, stage the result out."""
        sizes = h.tile_sizes
    ...
    with air.launch(name="eltwise_mul") as launch:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="air.api element-wise multiply example"
    )
    parser.add_argument(
        "--shape",
        type=int,
    ...
    # kernel registry records this kernel against. A bf16 product is exact to a
    # single bf16 round (the f32 product of two bf16 values is representable
    # before rounding), so atol is sized to the worst-case round -- the same
    # cleanest tier as element-wise add; f32 is effectively exact.
    ...
    module = launch.build(target=args.target)
    print(
        f"Eltwise mul shape={tuple(args.shape)} dtype={args.dtype} on {launch.target}"
    )
    runner = XRTRunner(
        verbose=args.verbose,
        omit_while_true_loop=False,
