    ...
    def copy(arg0, arg1):
        ...
        def launch_body(a, b):
            ...
            def segment_body():
                ...
                image_out_l2a = AllocOp(image_type_l2, [], [])
                image_out_l2b = AllocOp(image_type_l2, [], [])
                ...
                ChannelGet("ChanOutL1", image_out_l2a)
                # Access every value in the image
                for j in range_(IMAGE_HEIGHT):
                    for i in range_(IMAGE_WIDTH):
                        # Load the input value
                        val_in = load(image_out_l2a, [i, j])

                        # Calculate the output value
                        val_out = arith.addi(val_in, arith.ConstantOp(T.i32(), 1))

                        # Store the output value
                        store(val_out, image_out_l2b, [i, j])
                        yield_([])
                    yield_([])
                ChannelPut("ChanOutL2", image_out_l2b)

                DeallocOp(image_in_l2a)
                DeallocOp(image_in_l2b)
                DeallocOp(image_out_l2a)
                DeallocOp(image_out_l2b)
                """
                ...
                def herd_body(tx, ty, sx, sy):
                    ...
                    ChannelPut("ChanOutL2", image_out)
