...
# The Q4NX packing codec is the BUNDLE's format, not a per-model choice -- every
# FLM q4nx model, gemma4 included, packs identically -- so it is imported from
# the sibling engine rather than duplicated here. This is the only thing this
# builder borrows from fused_decode/; the rest of the fork stands alone.
import os.path as _osp
import sys as _sys

_sys.path.insert(
    0, _osp.join(_osp.dirname(_osp.abspath(__file__)), "..", "fused_decode")
)
...
_MODELS = {
    ...
    # ===== Gemma4-E2B (text): the model this builder exists for ============
    #
    # Two things make it structurally unlike every entry above, and both are
    # PER-LAYER rather than per-model:
    #
    #   1. TWO ATTENTION GEOMETRIES. Layers 4,9,...,34 are full attention with
    #      head_dim 512; the other 28 are sliding-window (window 512) with
    #      head_dim 256. FLM keeps BOTH paths resident in one design and picks
    #      per layer at runtime (RTP_*_IS_SWA_BUFFER).
    #   2. PER-LAYER EMBEDDINGS (PLE), the feature this builder is named for.
    #      A 256-wide per-layer vector is gated into the residual after the FFN.
    #      It is computed ONCE PER TOKEN from the embeddings, not per layer --
    #      getting that wrong is what cost the CPU reference layer 8 (0.79).
    #
    # THE TWO PER-LAYER SWITCHES ARE ONE BIT. Read off the bundle: layers <15
    # carry their own k/v AND a 6144 FFN; layers >=15 have NO k/v tensor at all
    # (they reuse the last non-shared layer of the same type) AND a 12288 FFN.
    # The wide FFN is spending exactly what the skipped KV projection saved,
    ...
    "gemma4-e2b": dict(
        ...
        HAS_QK_NORM=True,
        # --- PLE ---
        PLE_DIM=256,  # per_layer_input_dim
        # --- per-layer class map ---
        # Full-attention layers: layer_types is "ssssF" x7.
        FULL_LAYERS=(4, 9, 14, 19, 24, 29, 34),
        # Layers >= this reuse a lower layer's KV and carry the 12288 FFN.
        FIRST_KV_SHARED=15,
        INTER_NARROW=6144,  # layers <15; layers >=15 are 2x this
        VOCAB_SIZE=262144,
        UNI_DEC=35,  # 35 decoder layers
        # LM-head vocab chunking, by the same four rules as the entries above:
        # VOCAB_SIZE_PADDED_FULL = ceil(262144/1536)*1536 = 262656 -> 8208
        # rowblocks, and VOCAB_ROWBLKS = 16*VOCAB_I2 must divide it, so
        # UNI_LM*VOCAB_CHUNK_I2 = 513 = 3^3*19. K/PAYLOAD = 3 must divide
        # VOCAB_RNDS = VOCAB_I2, leaving {3,9,27,57,171,513}; the tested
        # 2*VOCAB_I2 <= 63 envelope caps it at 27, the largest legal chunk and
        # so the fewest host-armed waves: 513/27 = 19.
        # The driver MUST set VOCAB_CHUNK_I2=27 to match this UNI_LM.
        UNI_LM=19,  # vocab chunks per LM head (VOCAB_CHUNK_I2=27)
    ),
...
# DECODE_NO_PLE=1 builds the same model with the PLE branch left out. Not a
# shipping config -- the model is numerically wrong without it -- but it
...
if PLE:
    # Destination index on the shared @pleW packet channel, one per core.
    PLE_DEST_GATE, PLE_DEST_PROJ, PLE_DEST_UP = 0, 1, 2
    # Logit blocks the vocab arm relays, matching the rms core's own cadence.
...
def build_module():
    ...
    def build():
        ...
        if PLE:
            ...
            # 32 wide, not 1: a shim BD's transfer length must be a multiple
            # of 4 bytes and a lone bf16 is 2. The kernel reads element 0; the
            # rest of the slot is padding, which is why the DDR layout reserves
            # PLE_SCALE_PAD for it.
            plescale_l1 = MemRefType.get([PLE_SCALE_PAD], bf16, memory_space=l1)
        ...
        if PLE:
            ...
            # consumer and the per-arm fan-out happens there:
            #   decode -> @toPle  -> PLE gate core
            #   vocab  -> @pleOut -> host (the logits)
            # The vocab arm reuses @pleOut rather than taking a channel of its
            # own: col 3's shim has no spare S2MM for a second host drain, and
            # the two producers (relay memtile in vocab, PLE up core in decode)
            # are arm-exclusive, so the host sees exactly one of them per wave.
            # The rms core's own port split is untouched by this (layerOut on
            ...
            #
            # The shim has 2 MM2S per column, 16 across the device, and this
            # design already spends 14 (weights, rms, rope, KV). Eight separate
            # PLE feeds took it to 22 and air-to-aie refused to place them
            # ("failed to link to any shim dma allocation"). Demultiplexing by
            # destination index is the same trick @outY uses for the proj
            # egress, and costs one MM2S instead of eight.
            #
            # Each destination's stream is a SEQUENCE the core reads in order:
            ...
            if PLE_BYPASS != 5:
                # The proj core's singletons, kept OFF the weight channel. On it
                # they are two extra BD tasks, and AIR only self-loops a channel
                # with exactly one -- see the @pleW comment below. Circuit
                # switched, not a fourth packet destination: a second PACKET
                # flow into the same compute tile deadlocked 0/12 whatever it
                # carried and whichever shim port it landed on.
                channel_decl("pleX", size=[1])
                channel_decl(
                    "pleW",
                        ...
                        def _uni_dec():
                            ...
                            def _emit_ple_feed():
                                ...
                                def _feed_proj():
                                    ...
                                    if PLE_SHARED_RES:
                                        # The UP core's norm weight and scale.
                                        # They are the gate's to forward, but on
                                        # the gate's weight channel they are a
                    ...
                    if PLE:
                        ...
                        def plegate_h(tx, ty, _sx, _sy, _arm, *_sh):
                            def _dec():
                                ...
                                if PLE_SHARED_RES and not PLE_NO_PLIOUT:
