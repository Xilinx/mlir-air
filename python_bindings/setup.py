...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        MLIR_AIR_INSTALL_ABS_PATH = Path(mlir_air.__path__[0]).absolute()
        ...
        if platform.system() == "Windows":
            ...
            shutil.move(MLIR_AIE_INSTALL_ABS_PATH, "/tmp/e")
            MLIR_AIE_INSTALL_ABS_PATH = Path("/tmp/a").absolute()
            shutil.move(MLIR_AIR_INSTALL_ABS_PATH, "/tmp/a")
            MLIR_AIR_INSTALL_ABS_PATH = Path("/tmp/a").absolute()
            shutil.move(MLIR_INSTALL_ABS_PATH, "/tmp/m")
            MLIR_INSTALL_ABS_PATH = Path("/tmp/m").absolute()

        cmake_args = [
            f"-G {cmake_generator}",
            f"-DCMAKE_PREFIX_PATH={MLIR_AIR_INSTALL_ABS_PATH}",
            f"-DMLIR_DIR={MLIR_INSTALL_ABS_PATH / 'lib' / 'cmake' / 'mlir'}",
            f"-DAIE_DIR={MLIR_AIE_INSTALL_ABS_PATH / 'lib' / 'cmake' / 'aie'}",
            f"-DAIR_DIR={MLIR_AIR_INSTALL_ABS_PATH / 'lib' / 'cmake' / 'air'}",
            f"-DCMAKE_INSTALL_PREFIX={install_dir}",
            "-DLLVM_CCACHE_BUILD=ON",
            "-DAIR_ENABLE_BINDINGS_PYTHON=ON",
...
setup(
    version=version("mlir-air"),
    author="",
    name="air",
    include_package_data=True,
    author_email="maksim.levental@gmail.com",
    long_description_content_type="text/markdown",
    ext_modules=[CMakeExtension("_air", sourcedir=".")],
