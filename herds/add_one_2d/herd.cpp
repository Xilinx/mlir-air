...
void _mlir_ciface_air_shim_memcpy2d(uint32_t id, uint64_t x, uint64_t y, void* t, uint64_t offset_x, uint64_t offset_y, uint64_t length, uint64_t stride, uint64_t elem_per_stride) {
  //printf("Do transfer with id %d of length %ld on behalf of x=%ld, y=%ld using shim DMA %ld channel %ld, offset is [%ld, %ld]\n", id, length, x, y, TILE_TO_SHIM_DMA[id-1][x][y], ARG_TO_SHIM_DMA_CHANNEL[id-1][x][y], offset_x, offset_y);

  tensor_t<int32_t,2> *tt = (tensor_t<int32_t,2> *)t;
  //size_t height = tt->shape[0];
  size_t width = tt->shape[1];
  size_t offset = offset_y*width + offset_x;

  // Used to use BRAM_ADDR + 0x4000 as the data address
  uint64_t addr = (u64)BRAM_ADDR;
  int32_t *bounce_buffer = (int32_t *)bram_ptr;

  for (uint64_t s=0, e=length/elem_per_stride; s<e; s++) {

    if (id == 1) {
      // This is the input, so we need to take what is in t and put it into the BRAM
      for (int i=0; i<elem_per_stride; i++) {
        bounce_buffer[i] = tt->d[offset + stride*s + i];
      }
    }
    ...
    XAieDma_ShimBdSetAddr(&ShimDmaInst1, bd, HIGH_ADDR(addr), LOW_ADDR(addr), elem_per_stride*sizeof(uint32_t));
