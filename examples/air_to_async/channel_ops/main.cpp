...
template <typename T>
void _c_reference(tensor_t<T, 2> *a, tensor_t<T, 2> *b)
{
  T buffer[8][8];
...
int main(int argc, char *argv[])
{
  ...
  input.alloc = input.data = (int32_t *)malloc(
      sizeof(int32_t) * input.shape[0] * input.shape[1]);

  output.shape[0] = output.shape[1] = M_SIZE;
  output.alloc = output.data = (int32_t *)malloc(
      sizeof(int32_t) * output.shape[0] * output.shape[1]);

  golden.shape[0] = golden.shape[1] = M_SIZE;
  golden.alloc = golden.data = (int32_t *)malloc(
      sizeof(int32_t) * golden.shape[0] * golden.shape[1]);
