...
output_per_row=2
dram_bufsize_out=256*output_per_row*b_block_depth
...
// SPDX-License-Identifier: MIT\n\n\n""")
    ...
    for block in range (0, total_b_block):
            ...
            bb_sym = broad_in[:-1]
            # print(broad_in)
            f.write("  %%block_%d_buf_in_shim_%d = AIE.objectFifo.createObjectFifo(%%tile%d_0,{%s},%d :i32) { sym_name = \"%s\" } : !AIE.objectFifo<memref<%dxi32>> //B block input\n" \
             %(block, shim_place,shim_place, bb_sym,input_rows,  symbol_in, bufsize))
            
            for row in range (startrow, startrow+b_block_depth): #
                if(block%2==0):
                    col=startcol+0+b_col_shift*3
                    # broad_in= broad_in+("%%tile%d_%d," % (col,row))
                    f.write("  %%block_%d_buf_row_%d_inter_lap= AIE.objectFifo.createObjectFifo(%%tile%d_%d,{%%tile%d_%d},5:i32){ sym_name =\"block_%d_buf_row_%d_inter_lap\"} : !AIE.objectFifo<memref<%dxi32>>\n" \
                    %(block, row, col, row, col+1, row, block, row, bufsize))
                    
                    col=startcol+1+b_col_shift*3
                    f.write("  %%block_%d_buf_row_%d_inter_flx1= AIE.objectFifo.createObjectFifo(%%tile%d_%d,{%%tile%d_%d},6:i32) { sym_name =\"block_%d_buf_row_%d_inter_flx1\"} : !AIE.objectFifo<memref<%dxi32>>\n" \
                    %(block, row, col, row, col+1, row, block, row, bufsize_flx1))
                    col=startcol+2+b_col_shift*3
                 
                    if(row==startrow+b_block_depth-3):
                        f.write("  %%block_%d_buf_out_shim_%d= AIE.objectFifo.createObjectFifo(%%tile%d_%d,{%%tile%d_%d},5:i32){ sym_name =\"block_%d_buf_out_shim_%d\"} : !AIE.objectFifo<memref<%dxi32>> //B block output\n" \
                        %(block,  shim_place,col, row, shim_place, 0, block, shim_place, bufsize_flx2))
                        
                    else:
                        f.write("  %%block_%d_buf_row_%d_out_flx2= AIE.objectFifo.createObjectFifo(%%tile%d_%d,{%%tile%d_%d},2:i32) { sym_name =\"block_%d_buf_row_%d_out_flx2\"} : !AIE.objectFifo<memref<%dxi32>>\n" \
    ...
    def gagan_gen_lap_core(block, col, row,shim_place):
            ...
            f.write("    %%ub = arith.constant %d : index\n"%(output_per_row))
    ...
    for b in range (0, total_b_block): # col 0 is reserved in aie
        ...
        # for c in range (startcol, startcol + arraycols+hdiff_col): # col 0 is reserved in aie
            for r in range (startrow,startrow+broadcast_cores+b_block_depth):
                ...
                if(b%2==0):
                    gagan_gen_lap_core(b,startcol+0+b_col_shift*3,r,shim_place)
                    gagan_gen_flx1_core(b,startcol+1+b_col_shift*3,r,shim_place)
                    gagan_gen_flx2_core(b,startcol+2+b_col_shift*3,r,shim_place)
                else:
                    gagan_gen_lap_core(b,startcol+0+b_col_shift*3,r+b_row_shift*4,shim_place)
                    gagan_gen_flx1_core(b,startcol+1+b_col_shift*3,r+b_row_shift*4,shim_place)
                    gagan_gen_flx2_core(b,startcol+2+b_col_shift*3,r+b_row_shift*4,shim_place)
