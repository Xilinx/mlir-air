...
void matmul_vectorized_4x8x4_bf16_f32(const bfloat16 *__restrict pA,
  ...
  static_assert(k % (2 * s) == 0 && k / (2 * s) > 0);
  static_assert(n % (2 * t) == 0 && n / (2 * t) > 0);
  return matmul_vectorized_2x2<bfloat16, float, m / r, k / s, n / t, r, s, t>(
      pA, pB, pC);
}
