...
template <typename Tin, typename Tout>
void matmul_bias_relu(int M, int N, int K, const std::vector<Tin> A,
            const std::vector<Tin> B, const std::vector<Tin> BIAS, std::vector<Tout> &C) {
  // A is an  MxK matrix
  // B is a   KxN matrix
  // BIAS is a   N vector
  // C is the MxN output matrix, assumed to be zeroed out
  ...
  // Add BIAS
  C_ptr = C.data();
  const Tin *BIAS_ptr = BIAS.data();
  for (int row = 0; row < M; row++) {
    for (int col = 0; col < N; col++) {
      *C_ptr += Tout(BIAS_ptr[col]);
      C_ptr += 1;
    }
  }
