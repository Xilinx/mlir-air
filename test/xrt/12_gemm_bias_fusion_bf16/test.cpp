...
int main(int argc, const char *argv[]) {
  ...
  std::vector<BIAS_DATATYPE> BIASVec(BIAS_VOLUME);
  for (int i = 0; i < BIAS_VOLUME; i++) {
    BIASVec[i] = matmul_common::random_bfloat16_t();
  }
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    auto run = kernel(bo_instr, instr_v.size(), bo_a, bo_b, bo_bias, bo_c);
    run.wait();
    ...
    if (VERIFY) {
      ...
      matmul_common::matmul_bias_relu(M, N, K, AVec, BVec, BIASVec, CVecRef);
