...
#define A_DATATYPE int8_t
#define B_DATATYPE int8_t
          ...
          for (size_t cin = 0; cin < CHIN; cin++) {
            for (size_t ky = 0; ky < K; ky++) {
              for (size_t kx = 0; kx < K; kx++) {
                Tin _a = a[batch * CHIN * XIN * YIN + cin * XIN * YIN +
                           (y + ky) * XIN + x + kx];
                Tin _b = b[cout * CHIN * K * K + cin * K * K + ky * K + kx];
                r[idx] += _a * _b;
              }
            }
          }
...
template <typename Tin, typename Tout>
void conv_out_nhwc_hwcf(std::vector<Tin> a, std::vector<Tin> b,
                        std::vector<Tout> &r) {
  for (size_t batch = 0; batch < BATCH; batch++) {
    for (size_t cout = 0; cout < CHOUT; cout++) {
      for (size_t y = 0; y < YOUT; y++) {
        for (size_t x = 0; x < XOUT; x++) {
          ...
          for (size_t cin = 0; cin < CHIN; cin++) {
            for (size_t ky = 0; ky < K; ky++) {
              for (size_t kx = 0; kx < K; kx++) {
                Tin _a = a[batch * CHIN * XIN * YIN + (y + ky) * XIN * CHIN +
                           (x + kx) * CHIN + cin];
                Tin _b = b[ky * CHIN * CHOUT * K + kx * CHOUT * CHIN +
                           cin * CHOUT + cout];
                r[idx] += _a * _b;
              }
            }
...
int main(int argc, const char *argv[]) {
  ...
  for (int i = 0; i < A_VOLUME; i++) {
    AVec.push_back(rand() % 0xf);
  }
  memcpy(bufA, AVec.data(), (AVec.size() * sizeof(A_DATATYPE)));
  B_DATATYPE *bufB = bo_b.map<B_DATATYPE *>();
  std::vector<B_DATATYPE> BVec;
  for (int i = 0; i < B_VOLUME; i++) {
    BVec.push_back(rand() % 0xf);
  }
  ...
  float macs = 2.0 * float(BATCH) * float(CHIN) * float(CHOUT) * float(K) *
               float(K) * float(XOUT) * float(YOUT);
  ...
  std::cout << std::endl
            << "NPU matmul time: " << npu_time_total << "us." << std::endl;
