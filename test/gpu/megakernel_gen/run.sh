...
echo "GFX_TARGET=$GFX_TARGET LAYERS=$LAYERS DIM=$DIM TASKS=$TASKS WORKERS=$WORKERS REPEAT=$REPEAT"

python3 "$SCRIPT_DIR/gen.py" --layers "$LAYERS" --dim "$DIM" --tasks "$TASKS" --workers "$WORKERS" --repeat "$REPEAT" > "$TMPDIR/chain.mlir"
