...
python3 "$SCRIPT_DIR/gen.py" --layers "$LAYERS" --dim "$DIM" --tasks "$TASKS" --workers "$WORKERS" --repeat "$REPEAT" --tokens "$TOKENS" --steps "$STEPS" --prompt-len "$PROMPT_LEN" --waves "${WAVES:-1}" > "$TMPDIR/chain.mlir"
