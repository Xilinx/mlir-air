...
echo "GFX_TARGET=$GFX_TARGET LAYERS=$LAYERS DIM=$DIM TASKS=$TASKS REPEAT=$REPEAT"

python3 "$SCRIPT_DIR/gen.py" --layers "$LAYERS" --dim "$DIM" --tasks "$TASKS" --repeat "$REPEAT" > "$TMPDIR/chain.mlir"
