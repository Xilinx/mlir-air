  ...
  func.func @main() {
    ...
    air.rank (%rid) in (%rsize = %c2) {
      ...
      scf.if %is_producer {
        ...
        // 1 block, 1 segment, 1 herd PE — single warp executes the body.
        ...
        air.launch (%bx) in (%nx = %p_c1)
          ...
          air.segment args(%sdata = %ldata, %sbases = %lbases)
            ...
            air.herd tile (%tx, %ty) in (%ntx = %p_c1_s, %nty = %p_c1_s)
              ...
              %tid = gpu.thread_id x
              %active = arith.cmpi ult, %tid, %c32_h : index
              %peer_data = air.translate %hdata, %from, %to, %hbases
                  : memref<32xi32, #air.symmetric_heap>,
                    memref<?xindex, #air.symmetric_heap>

              scf.if %active {
                %is_flag  = arith.cmpi eq, %tid, %c31_h : index
                ...
                memref.store %val, %peer_data[%tid]
                    : memref<32xi32, #air.symmetric_heap>
              }
              air.herd_terminator
            }
