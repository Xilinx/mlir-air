...
module {
  ...
  func.func @forward(%arg0: memref<64x64xi32>, %arg1: memref<64x64xi32>, %arg2: memref<64x64xi32>) {
    ...
    %1 = scf.parallel (%arg3, %arg4) = (%c0, %c0) to (%c2, %c2) step (%c1, %c1) init (%0) -> !air.async.token {
      ...
      %7 = scf.for %arg5 = %c0 to %c64 step %c32 iter_args(%arg6 = %async_token_5) -> (!air.async.token) {
        %8 = air.channel.put async [%arg6]  @channel_0[%arg3, %arg4] (%arg0[%results_6, %arg5] [%c32, %c32] [%c64, %c1]) {id = 1 : i32} : (memref<64x64xi32>)
    ...
    %3 = scf.parallel (%arg3, %arg4) = (%c0, %c0) to (%c2, %c2) step (%c1, %c1) init (%2) -> !air.async.token {
      ...
      %7 = scf.for %arg5 = %c0 to %c64 step %c32 iter_args(%arg6 = %async_token_5) -> (!air.async.token) {
        %8 = air.channel.put async [%arg6]  @channel_1[%arg3, %arg4] (%arg1[%arg5, %results_6] [%c32, %c32] [%c64, %c1]) {id = 2 : i32} : (memref<64x64xi32>)
        scf.yield %8 : !air.async.token
      }
      scf.reduce(%7)  : !air.async.token {
      ^bb0(%arg5: !air.async.token, %arg6: !air.async.token):
    ...
    %4 = scf.parallel (%arg3, %arg4) = (%c0, %c0) to (%c2, %c2) step (%c1, %c1) init (%async_token_3) -> !air.async.token {
      %async_token_5, %results_6 = air.execute -> (index) {
      ...
      %7 = air.wait_all async [%async_token_7, %async_token_5, %async_token_3] 
      %8 = scf.for %arg5 = %c0 to %c64 step %c32 iter_args(%arg6 = %7) -> (!air.async.token) {
        %9 = air.channel.put async [%arg6]  @channel_2[%arg3, %arg4] (%results_2[%results_6, %results_8] [%c32, %c32] [%c64, %c1]) {id = 3 : i32} : (memref<64x64xi32>)
    ...
    %5 = scf.parallel (%arg3, %arg4) = (%c0, %c0) to (%c2, %c2) step (%c1, %c1) init (%async_token_3) -> !air.async.token {
      ...
      %7 = air.wait_all async [%async_token_7, %async_token_5, %async_token_3] 
      %8 = scf.for %arg5 = %c0 to %c64 step %c32 iter_args(%arg6 = %7) -> (!air.async.token) {
        %9 = air.channel.get async [%arg6]  @channel_3[%arg3, %arg4] (%results_2[%results_6, %results_8] [%c32, %c32] [%c64, %c1]) {id = 4 : i32} : (memref<64x64xi32>)
        scf.yield %9 : !air.async.token
      }
      scf.reduce(%8)  : !air.async.token {
      ^bb0(%arg5: !air.async.token, %arg6: !air.async.token):
    ...
    %6 = air.herd @herd_0 async [%async_token_3]  tile (%arg3, %arg4) in (%arg5=%c2, %arg6=%c2) attributes {id = 1 : i32} {
      ...
      %8 = scf.for %arg7 = %c0_5 to %c64_6 step %c32_7 iter_args(%arg8 = %7) -> (!air.async.token) {
        ...
        %9 = air.channel.get async [%async_token_8, %arg8]  @channel_0[%arg3, %arg4] (%results_9[] [] []) {id = 5 : i32} : (memref<32x32xi32, 2>)
        %10 = air.channel.get async [%async_token_10, %arg8]  @channel_1[%arg3, %arg4] (%results_11[] [] []) {id = 6 : i32} : (memref<32x32xi32, 2>)
        %11 = air.channel.get async [%async_token_12, %arg8]  @channel_2[%arg3, %arg4] (%results_13[] [] []) {id = 7 : i32} : (memref<32x32xi32, 2>)
        %async_token_14 = air.execute [%11, %10, %9] {
          linalg.matmul {cast = #linalg.type_fn<cast_signed>} ins(%results_9, %results_11 : memref<32x32xi32, 2>, memref<32x32xi32, 2>) outs(%results_13 : memref<32x32xi32, 2>)
        }
        %12 = air.channel.put async [%async_token_14]  @channel_3[%arg3, %arg4] (%results_13[] [] []) {id = 8 : i32} : (memref<32x32xi32, 2>)
        ...
        %async_token_17 = air.execute [%12] {
          memref.dealloc %results_13 : memref<32x32xi32, 2>
        }
        %wait = air.wait_all async [%async_token_15, %async_token_16, %async_token_17]
