...
extern "C" void mgpuEventCreate(hipEvent_t event) {
//  hipEvent_t event = nullptr;
 // HIP_REPORT_IF_ERROR(hipEventCreateWithFlags(&event, hipEventDisableTiming));
  return HIP_REPORT_IF_ERROR(hipEventCreate(&event));
}
...
extern "C" void mgpuEventElapsedTime(float *ms, hipEvent_t start, hipEvent_t stop) {
  HIP_REPORT_IF_ERROR(hipEventElapsedTime(ms, start, stop));
...
extern "C" void mgpuCheckOutput(float *device, float* hostA, float* hostB, int N, int M){
    ...
    for(int j = 0; j<M; j++){
      if (std::fabs(hostA[i*M+j] - device[i*M+j]) > epsilon) {
                std::cout << "Mismatch at (" << i << ", " << j << "): "
                          << hostA[i*M+j] << " != " << device[i*M+j] << std::endl;
                mismatch = true;
                return;
      }
    }
...
extern "C" void mgpuInit(float *A, float* B, int N, int M){
  ...
  for (int y = 0; y < N; y++)
  {
    for (int x = 0; x < N; x++)
    {
      A[N * y + x] = static_cast<float>(i);
      B[N * y + x] = x == y ? 1.0f : 0.0f;
      i = i + 1;
    }
  }
  /*
