...
int main(int argc, char *argv[]) {
  ...
  auto graph_fn = (void (*)(void *, void *, void *))dlsym((void *)handle,
                                                          "_mlir_ciface_graph");
  assert(graph_fn && "failed to locate _mlir_ciface_graph in aie_ctrl.so");

  tensor_t<uint32_t, 2> input;
  tensor_t<uint32_t, 2> output;
  input.shape[0] = DMA_1D * 4;
  input.shape[1] = DMA_2D * 2;
  output.shape[0] = DMA_1D * 4;
  output.shape[1] = DMA_2D * 2;
  ...
  l2_input.shape[0] = DMA_1D;
  l2_input.shape[1] = DMA_2D;
