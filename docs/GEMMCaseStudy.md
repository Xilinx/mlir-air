...
The broadcast set annotations specify the conditions under which a particular broadcasting behavior should be applied, which is derived from the analysis of the `broadcast_pattern` attribute generated by an upstream pass (`air-dependency-schedule-opt`). 
